...
ROOT=Path('data/audit/verified-pbp')
TOURS=('ATP_MAIN','WTA_MAIN')
YEARS=range(2012,datetime.now(timezone.utc).year+1)
...
def aggregate():
 ...
 nxt=next_batch()
 completed={t:[y for y in YEARS if load(t,y)] for t in TOURS}
 partial={t:[] for t in TOURS}
 progress={'updated_at_utc':datetime.now(timezone.utc).isoformat(),'completed_batches':len(rows),'total_batches':len(TOURS)*len(list(YEARS)),'completed_years':completed,'partially_processed_years':partial,'totals':totals,'next_resume_position':({'tour':nxt[0],'year':nxt[1]} if nxt else None),'pipeline_complete':nxt is None}
 ROOT.mkdir(parents=True,exist_ok=True);(ROOT/'progress.json').write_text(json.dumps(progress,indent=2)+'\n')
 lines=['# Verified PBP Pipeline Progress','',f"Updated: {progress['updated_at_utc']}",'',f"Completed batches: {progress['completed_batches']} / {progress['total_batches']}",'',f"Next resume position: {progress['next_resume_position']}",'','## ATP MAIN by year','', '| Year | Historical Matches | PBP Candidates | Verified | Partial | Ambiguous | Conflicts | No PBP | Coverage % |','|---:|---:|---:|---:|---:|---:|---:|---:|---:|']
 for t in TOURS:
  ...
  for y in YEARS:
   s=load(t,y)
   if not s:continue
   lines.append(f"| {y} | {s['historical_matches']} | {s['pbp_candidates']} | {s['verified']} | {s['partial']} | {s['ambiguous']} | {s['conflicts']} | {s['no_pbp']} | {s['coverage_pct']:.2f} |")
...
if __name__=='__main__':
 p=aggregate()
 if '--next' in sys.argv:
  n=p['next_resume_position'];print('COMPLETE' if not n else f"{n['tour']} {n['year']}")
