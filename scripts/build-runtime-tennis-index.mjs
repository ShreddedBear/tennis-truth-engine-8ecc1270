...
function compactDetails(row,orientation='PLAYER'){
  ...
  if(orientation==='HOME'){sf=num(first(row,['home_sets','home_score_sets','sets_home']));sa=num(first(row,['away_sets','away_score_sets','sets_away']));}
  const rawScore=first(row,['score','scoreline','match_score','result_score','set_score','sets_score']);let setScores=parseSetScores(rawScore);
  if(orientation==='PLAYER'&&won===false)setScores=setScores.map(([a,b])=>[b,a]);
  const status=first(row,['status','match_status','result_status','retirement_status','walkover_status','comment']);
  const bestOf=num(first(row,['best_of','bestof','format_best_of']));
  const opponentRank=num(first(row,['opponent_rank','opp_rank','loser_rank']));
  const opponentElo=num(first(row,['opponent_elo','opp_elo','loser_elo']));
  return {sets_for:sf,sets_against:sa,set_scores:setScores,best_of:bestOf,opponent_rank:opponentRank,opponent_elo:opponentElo,status:status?String(status):null,raw_score:rawScore?String(rawScore):null};
}
function reverseDetails(d){return{...d,sets_for:d.sets_against,sets_against:d.sets_for,set_scores:Array.isArray(d.set_scores)?d.set_scores.map(([a,b])=>[b,a]):[],opponent_rank:null,opponent_elo:null};}
...
function addHistory(lane,player,opponent,date,tournament,surface,won,round,source,details=null){const key=norm(player);if(!key||!date)return;const bucket=matchHistory[lane][key]??=[];bucket.push([String(date),String(tournament??''),String(surface??''),String(opponent??''),won===true?1:won===false?0:null,String(round??''),String(source??''),details]);}
