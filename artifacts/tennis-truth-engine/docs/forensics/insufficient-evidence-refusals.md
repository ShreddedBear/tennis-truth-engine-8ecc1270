...
Snapshot: **SLATE 1 (prediction_slates.slate_number=1, 60 matches, project qyovnrkiknsiqjybxubf)**, captured 2026-09-10T14:51:20.447451+00:00. Read-only; no production data was modified.

Active metric set: 25 codes. Selection threshold: **60%** (unchanged by this audit).

Independent reconstruction agrees with the stored verdict in 32/32 cases; P1/P2 mirror-symmetry holds in 32/32.
...
| stress: support before → after | 40% → 33.3333% | 40% → 14.2857% |
| stress: status | FRAGILE | FRAGILE |
| stress: outcome when THIS side is stressed | INSUFFICIENT | INSUFFICIENT |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | RECENT_FORM | LOSS_PROFILE, RECENT_FORM |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 25% → 16.6667% | 25% → 25% |
| stress: status | REVERSED | REMOVED |
| stress: outcome when THIS side is stressed | P2 | INSUFFICIENT |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | LOSS_PROFILE, SURFACE_STRENGTH | — |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 20% → 20% | 40% → 20% |
| stress: status | REVERSED | FRAGILE |
| stress: outcome when THIS side is stressed | P2 | INSUFFICIENT |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | — | POINT_BY_POINT |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 50% → 0% | 33.3333% → 25% |
| stress: status | REVERSED | REVERSED |
| stress: outcome when THIS side is stressed | P2 | P1 |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | POINT_BY_POINT | POINT_BY_POINT, SET_PROFILE |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 50% → 40% | 16.6667% → 0% |
| stress: status | FRAGILE | REVERSED |
| stress: outcome when THIS side is stressed | INSUFFICIENT | P1 |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | — | IMPROVEMENT_TREND |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 33.3333% → 25% | 50% → 16.6667% |
| stress: status | REVERSED | REVERSED |
| stress: outcome when THIS side is stressed | P2 | P1 |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | LOSS_PROFILE, POINT_BY_POINT | CLOSING_ABILITY, POINT_BY_POINT |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 50% → 0% | 25% → 25% |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 16.6667% → 16.6667% | 50% → 50% |
| stress: status | REMOVED | REMOVED |
| stress: outcome when THIS side is stressed | INSUFFICIENT | INSUFFICIENT |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | — | — |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 50% → 33.3333% | 25% → 0% |
| stress: status | FRAGILE | REVERSED |
| stress: outcome when THIS side is stressed | INSUFFICIENT | P1 |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | IMPROVEMENT_TREND, LOSS_PROFILE | — |
...
| underdog: evaluated by production | false | false |
| stress: support before → after | 16.6667% → 16.6667% | 50% → 42.8571% |
| stress: status | REMOVED | FRAGILE |
| stress: outcome when THIS side is stressed | INSUFFICIENT | INSUFFICIENT |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | — | POINT_BY_POINT |
...
- stored: **INSUFFICIENT EVIDENCE** / colour INSUFFICIENT EVIDENCE / independent_winner null
- stage that caused the refusal: **STRESS (adverse one-noise-floor recomputation)**
...
| stress: support before → after | 0% → 0% | 66.6667% → 50% |
| stress: status | REVERSED | FRAGILE |
| stress: outcome when THIS side is stressed | P2 | INSUFFICIENT |
| stress: run by production | false | true |
| stress: families manufactured out of NEUTRAL | IMPROVEMENT_TREND, LOSS_PROFILE, POINT_BY_POINT | POINT_BY_POINT |
...
**Stage trace** — family vote `P2` → threshold `P2` → leave-one-family-out `P2` → verification `P2` → disagreement `P2` → underdog `P2` → stress `INSUFFICIENT` → stored `INSUFFICIENT`. Symmetric stress verdict: `LEADER_MORE_ROBUST`.

**Why:** The decision core selected P2 at 66.666667% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: LEADER_MORE_ROBUST), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were left intact, while 1 family/families that had voted for NOBODY (POINT_BY_POINT) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P2.
...
| stress: support before → after | 66.6667% → 20% | 33.3333% → 16.6667% |
| stress: status | REVERSED | REVERSED |
| stress: outcome when THIS side is stressed | P2 | P1 |
| stress: run by production | true | false |
| stress: families manufactured out of NEUTRAL | COMMON_OPPONENT, SURFACE_STRENGTH | COMMON_OPPONENT, IMPROVEMENT_TREND, POINT_BY_POINT |

Conflicted families: — · neutral: COMMON_OPPONENT, IMPROVEMENT_TREND, POINT_BY_POINT, SURFACE_STRENGTH · directional denominator: 3

**Stage trace** — family vote `P1` → threshold `P1` → leave-one-family-out `P1` → verification `P1` → disagreement `P1` → underdog `P1` → stress `P2` → stored `INSUFFICIENT`. Symmetric stress verdict: `NON_DISCRIMINATING`.

**Why:** The decision core selected P1 at 66.666667% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: NON_DISCRIMINATING), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were left intact, while 2 family/families that had voted for NOBODY (COMMON_OPPONENT, SURFACE_STRENGTH) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P1.
...
| underdog: evaluated by production | true | false |
| stress: support before → after | 0% → 0% | 66.6667% → 0% |
| stress: status | REVERSED | FRAGILE |
| stress: outcome when THIS side is stressed | P2 | INSUFFICIENT |
| stress: run by production | false | true |
| stress: families manufactured out of NEUTRAL | IMPROVEMENT_TREND, LOSS_PROFILE | SURFACE_STRENGTH |
...
**Why:** The decision core selected P2 at 66.666667% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: LEADER_MORE_ROBUST), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were left intact, while 1 family/families that had voted for NOBODY (SURFACE_STRENGTH) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P2.
...
| underdog: evaluated by production | false | true |
| stress: support before → after | 60% → 33.3333% | 40% → 0% |
| stress: status | REVERSED | REVERSED |
| stress: outcome when THIS side is stressed | P2 | P1 |
| stress: run by production | true | false |
| stress: families manufactured out of NEUTRAL | COMMON_OPPONENT, PSYCH_RESPONSE | SURFACE_STRENGTH |

Conflicted families: — · neutral: COMMON_OPPONENT, H2H_PROBABILITY, PSYCH_RESPONSE, SURFACE_STRENGTH · directional denominator: 5

**Stage trace** — family vote `P1` → threshold `P1` → leave-one-family-out `P1` → verification `P1` → disagreement `P1` → underdog `P1` → stress `P2` → stored `INSUFFICIENT`. Symmetric stress verdict: `NON_DISCRIMINATING`.

**Why:** The decision core selected P1 at 60.000000% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: NON_DISCRIMINATING), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were neutralised (RECENT_FORM), while 2 family/families that had voted for NOBODY (COMMON_OPPONENT, PSYCH_RESPONSE) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P1.
...
| underdog: evaluated by production | false | true |
| stress: support before → after | 71.4286% → 40% | 28.5714% → 25% |
| stress: status | REVERSED | REVERSED |
| stress: outcome when THIS side is stressed | P2 | P1 |
| stress: run by production | true | false |
| stress: families manufactured out of NEUTRAL | IMPROVEMENT_TREND, POINT_BY_POINT, PSYCH_RESPONSE | POINT_BY_POINT |

Conflicted families: — · neutral: IMPROVEMENT_TREND, POINT_BY_POINT, PSYCH_RESPONSE · directional denominator: 7

**Stage trace** — family vote `P1` → threshold `P1` → leave-one-family-out `P1` → verification `P1` → disagreement `P1` → underdog `P1` → stress `P2` → stored `INSUFFICIENT`. Symmetric stress verdict: `NON_DISCRIMINATING`.

**Why:** The decision core selected P1 at 71.428571% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: NON_DISCRIMINATING), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were left intact, while 3 family/families that had voted for NOBODY (IMPROVEMENT_TREND, POINT_BY_POINT, PSYCH_RESPONSE) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P1.
...
| underdog: evaluated by production | true | false |
| stress: support before → after | 40% → 16.6667% | 60% → 12.5% |
| stress: status | REVERSED | REVERSED |
| stress: outcome when THIS side is stressed | P2 | P1 |
| stress: run by production | false | true |
| stress: families manufactured out of NEUTRAL | PSYCH_RESPONSE | IMPROVEMENT_TREND, LOSS_PROFILE, RESULTS_HISTORY |
...
**Stage trace** — family vote `P2` → threshold `P2` → leave-one-family-out `P2` → verification `P2` → disagreement `P2` → underdog `P2` → stress `P1` → stored `INSUFFICIENT`. Symmetric stress verdict: `NON_DISCRIMINATING`.

**Why:** The decision core selected P2 at 60.000000% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: NON_DISCRIMINATING), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were left intact, while 3 family/families that had voted for NOBODY (IMPROVEMENT_TREND, LOSS_PROFILE, RESULTS_HISTORY) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P2.
...
| underdog: evaluated by production | false | true |
| stress: support before → after | 80% → 33.3333% | 20% → 14.2857% |
| stress: status | REVERSED | REVERSED |
| stress: outcome when THIS side is stressed | P2 | P1 |
| stress: run by production | true | false |
| stress: families manufactured out of NEUTRAL | IMPROVEMENT_TREND, RECENT_FORM | CLOSING_ABILITY, SET_PROFILE |

Conflicted families: — · neutral: CLOSING_ABILITY, IMPROVEMENT_TREND, RECENT_FORM, SET_PROFILE · directional denominator: 5

**Stage trace** — family vote `P1` → threshold `P1` → leave-one-family-out `P1` → verification `P1` → disagreement `P1` → underdog `P1` → stress `P2` → stored `INSUFFICIENT`. Symmetric stress verdict: `NON_DISCRIMINATING`.

**Why:** The decision core selected P1 at 80.000000% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: NON_DISCRIMINATING), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were neutralised (SURFACE_STRENGTH), while 2 family/families that had voted for NOBODY (IMPROVEMENT_TREND, RECENT_FORM) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P1.
...
| stress: outcome when THIS side is stressed | P2 | INSUFFICIENT |
| stress: run by production | false | true |
| stress: families manufactured out of NEUTRAL | LOSS_PROFILE | — |

Conflicted families: COMMON_OPPONENT · neutral: H2H_PROBABILITY, LOSS_PROFILE · directional denominator: 4

**Stage trace** — family vote `P2` → threshold `P2` → leave-one-family-out `P2` → verification `P2` → disagreement `P2` → underdog `P2` → stress `INSUFFICIENT` → stored `INSUFFICIENT`. Symmetric stress verdict: `LEADER_MORE_ROBUST`.

**Why:** The decision core selected P2 at 75.000000% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: LEADER_MORE_ROBUST), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were neutralised (RECENT_FORM, SURFACE_STRENGTH), and no new opposing family was created. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P2.
...
**Why:** The decision core selected P2 at 66.666667% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: LEADER_MORE_ROBUST), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were left intact, and no new opposing family was created. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P2.
...
| underdog: evaluated by production | false | true |
| stress: support before → after | 71.4286% → 57.1429% | 14.2857% → 0% |
| stress: status | FRAGILE | REVERSED |
| stress: outcome when THIS side is stressed | INSUFFICIENT | P1 |
| stress: run by production | true | false |
| stress: families manufactured out of NEUTRAL | POINT_BY_POINT | POINT_BY_POINT, PSYCH_RESPONSE |

Conflicted families: COMMON_OPPONENT · neutral: H2H_PROBABILITY, POINT_BY_POINT, PSYCH_RESPONSE · directional denominator: 7

**Stage trace** — family vote `P1` → threshold `P1` → leave-one-family-out `P1` → verification `P1` → disagreement `P1` → underdog `P1` → stress `INSUFFICIENT` → stored `INSUFFICIENT`. Symmetric stress verdict: `LEADER_MORE_ROBUST`.

**Why:** The decision core selected P1 at 71.428571% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: LEADER_MORE_ROBUST), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were neutralised (CLOSING_ABILITY), while 1 family/families that had voted for NOBODY (POINT_BY_POINT) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P1.
...
| underdog: evaluated by production | false | true |
| stress: support before → after | 100% → 0% | 0% → 0% |
| stress: status | FRAGILE | REVERSED |
| stress: outcome when THIS side is stressed | INSUFFICIENT | P1 |
| stress: run by production | true | false |
| stress: families manufactured out of NEUTRAL | — | — |
...
**Why:** The decision core selected P1 at 100.000000% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: LEADER_MORE_ROBUST), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were left intact, and no new opposing family was created. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P1.
...
| underdog: evaluated by production | true | false |
| stress: support before → after | 0% → 0% | 80% → 57.1429% |
| stress: status | REVERSED | FRAGILE |
| stress: outcome when THIS side is stressed | P2 | INSUFFICIENT |
| stress: run by production | false | true |
| stress: families manufactured out of NEUTRAL | RECENT_FORM, SET_PROFILE | CLOSING_ABILITY, RESULTS_HISTORY |
...
**Why:** The decision core selected P2 at 80.000000% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: LEADER_MORE_ROBUST), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were left intact, while 2 family/families that had voted for NOBODY (CLOSING_ABILITY, RESULTS_HISTORY) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P2.
...
| underdog: evaluated by production | false | true |
| stress: support before → after | 100% → 40% | 0% → 0% |
| stress: status | FRAGILE | REVERSED |
| stress: outcome when THIS side is stressed | INSUFFICIENT | P1 |
| stress: run by production | true | false |
| stress: families manufactured out of NEUTRAL | COMMON_OPPONENT | — |

Conflicted families: — · neutral: COMMON_OPPONENT, H2H_PROBABILITY · directional denominator: 5

**Stage trace** — family vote `P1` → threshold `P1` → leave-one-family-out `P1` → verification `P1` → disagreement `P1` → underdog `P1` → stress `INSUFFICIENT` → stored `INSUFFICIENT`. Symmetric stress verdict: `LEADER_MORE_ROBUST`.

**Why:** The decision core selected P1 at 100.000000% of the directional evidence, cleared the 60% threshold and survived leave-one-family-out; the STRESS stage then converted that selection into a refusal. The conversion is not a comparative finding: the adverse case is applied ONLY to the selected side (symmetric verdict: LEADER_MORE_ROBUST), so the opponent's profile is never subjected to the same erosion before the selection is withdrawn. Under the adverse case the leader's own families were neutralised (LOSS_PROFILE), while 1 family/families that had voted for NOBODY (COMMON_OPPONENT) were converted into votes for the opponent -- directional evidence the shift created out of measured parity, not an eroded edge. Mirror case: with the CHALLENGER's edges eroded instead, the outcome is P1.
...
| underdog: evaluated by production | false | true |
| stress: support before → after | 60% → 33.3333% | 20% → 20% |
| stress: status | FRAGILE | REVERSED |
| stress: outcome when THIS side is stressed | INSUFFICIENT | P1 |
| stress: run by production | true | false |
| stress: families manufactured out of NEUTRAL | LOSS_PROFILE | — |
...
| stress: outcome when THIS side is stressed | P2 | INSUFFICIENT |
| stress: run by production | false | false |
| stress: families manufactured out of NEUTRAL | SURFACE_STRENGTH | — |
