...
HEAD={'Authorization':f'Token {TOKEN}','User-Agent':'tennis-truth-engine-bsd-wta-history/1.0'}
OUT=Path('data/audit/bsd-wta-main-pbp-history')
PAGE_SIZE=200
MAX_PAGES=100
...
def wta_main(m):
    """Strict WTA-main classifier. Never infer WTA merely from female gender."""
    ...
    if circuit != 'WTA':
        return False
    exclusions=('challenger','125','wta 125','125k','utr','itf','futures','satellite','exhibition')
...
def year_scan(year):
    ...
    summary={
        'year':year,'matches_listed_all_tours':listed,'wta_main_matches':classified,
        'wta_main_pbp_requests':len(rows),'pbp_available':len(good),
        'no_pbp_or_unusable':len(rows)-len(good),'list_failures':list_failures,
        'scan_valid_for_boundary':not list_failures,
    ...
    (d/'UPDATE.md').write_text(
        f"* {classified:,} strict WTA Main finished matches examined for {year}.\n"
        f"* {len(good):,} BSD matches returned usable point-by-point data.\n"
        f"* {len(rows)-len(good):,} returned no usable PBP.\n"
        f"* Earliest confirmed BSD WTA Main PBP date in {year}: {summary['earliest_confirmed_pbp_date'] or 'NONE'}.\n"
        f"* No second PBP provider was used; this is a BSD-only availability/structure scan.\n"
    )
    return summary
