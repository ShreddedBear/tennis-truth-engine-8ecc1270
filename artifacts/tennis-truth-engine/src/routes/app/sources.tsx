...
function Sources() {
  ...
  const { data } = useQuery({
    queryKey: ["sources"],
    queryFn: async () => {
      const response = await truthApi.getSourcesScreen();
      return { snapshots: response.snapshots as any[], conflicts: response.conflicts as any[] };
    },
  });

  const resolve = async (id: string, resolution: string) => {
    await truthApi.resolveSourceConflict(id, resolution);
