...
import { truthApi, uploadTruthSummary } from "@/lib/truth-api-client";
...
function UploadPage(){
 ...
 const visionExtract=useServerFn(extractMatchupsFromPdf);const resolveContext=useServerFn(resolveMatchContext);const executeBatch=useServerFn(runAuditBatch);
 ...
 const analyze=async()=>{if(!files.length)return;setBusy(true);try{const next:Staged[]=[];for(const file of files){setProgress(`Reading ${file.name}…`);let pages:string[]=[];let matchups:ParsedMatchup[]=[];let source:Staged["source"]="TEXT";const isLarge=file.size>LARGE_PDF_SKIP_TEXT_EXTRACTION_BYTES;if(isLarge){toast.info(`${file.name} is ${(file.size/1024/1024).toFixed(1)}MB — likely a screenshot compilation, going straight to on-device OCR. Large batches (30+ matches) are more reliable split into a few smaller uploads.`);}else{try{pages=(await extractPdfText(file)).pages;matchups=parseSummaryText(pages);}catch(e){pages=[];recordError("PDF TEXT EXTRACTION",e,{file:file.name});}}if(matchups.length===0){setProgress(`${file.name}: running free local OCR…`);try{const ocr=await ocrPdfLocally(file,m=>setProgress(`${file.name}: ${m}`));pages=ocr.pages;matchups=parseSummaryText(pages);source="LOCAL_OCR";}catch(e){recordError("LOCAL OCR",e,{file:file.name});}}if(matchups.length===0&&!isLarge){setProgress(`${file.name}: local OCR found no matchup — trying vision extraction…`);try{const base64=await toBase64(file);const{matchups:ai}=await visionExtract({data:{filename:file.name,base64}});matchups=ai.map(aiToParsed);source="VISION";}catch(e){const message=recordError("VISION EXTRACTION",e,{file:file.name});if(/402|credit/i.test(message))toast.error(`${file.name}: local OCR could not identify the matchup and AI credits are exhausted.`);else toast.error(`${file.name}: ${message}`);}}matchups=dedupeMatchups(matchups);if(!matchups.length){recordError("MATCHUP DETECTION","No matchups detected — review required",{file:file.name});toast.warning(`${file.name}: no matchups detected — review required`);}next.push({filename:file.name,pages,matchups,source});}await enrich(next);for(const f of next)f.matchups=dedupeMatchups(f.matchups);setStaged(s=>[...s,...next]);setFiles([]);}catch(e){const message=recordError("START ANALYSIS",e);toast.error(`Analysis failed: ${message}`);}finally{setProgress(null);setBusy(false);}};
  const editField=(fi:number,mi:number,key:string,value:string)=>setStaged(s=>s.map((f,i)=>i!==fi?f:{...f,matchups:f.matchups.map((m,j)=>{if(j!==mi)return m;const exists=m.fields.some(x=>x.field_key===key);const provenance={...(m.metadata_provenance??{}),[key]:{source:"manual OCR review",method:"OCR",status:"VERIFIED",direct:true}};const fields=(exists?m.fields.map(x=>x.field_key===key?{...x,normalized_value:value,extraction_status:"DIRECT" as const}:x):[...m.fields,{field_key:key,raw_value:null,normalized_value:value,extraction_status:"DIRECT" as const,confidence:1,page_number:m.page_number}]).map(x=>x.field_key==="__metadata_provenance"?{...x,normalized_value:JSON.stringify(provenance)}:x);return{...m,fields,metadata_provenance:provenance};})}));
 ...
 const fieldValue=(m:ParsedMatchup,key:string)=>m.fields.find(f=>f.field_key===key)?.normalized_value??"";
  const commit=async()=>{setBusy(true);let created=0,versions=0;const auditedMatchIds=new Set<string>();try{for(const file of staged){for(const m of dedupeMatchups(file.matchups)){const matchLabel=`${m.player1_name} vs ${m.player2_name}`;try{const key=canonicalKey({tournament:fieldValue(m,"tournament")||null,round:fieldValue(m,"round")||null,date:fieldValue(m,"scheduled_date")||null,p1:m.player1_name,p2:m.player2_name});const result=await uploadTruthSummary({filename:file.filename,rawText:file.pages.join("\n\f\n"),pageCount:file.pages.length,match:{canonicalKey:key,player1Name:m.player1_name,player2Name:m.player2_name,tournamentName:fieldValue(m,"tournament")||null,eventLevel:fieldValue(m,"event_level")||null,round:fieldValue(m,"round")||null,scheduledDate:fieldValue(m,"scheduled_date")||null,surface:fieldValue(m,"surface")||null,bestOf:Number(fieldValue(m,"best_of"))||null},fields:m.fields.map(f=>({fieldKey:f.field_key,rawValue:f.raw_value,normalizedValue:f.normalized_value,fieldType:f.field_key}))});const matchId=(result.match as {id?:string}).id;if(!matchId)throw new Error("API did not return an ingested match id");created++;versions++;await log({match_id:matchId,stage:"SUMMARY PDF INGESTION",status:"COMPLETE",output:{file:file.filename,page:m.page_number,source:file.source}});auditedMatchIds.add(matchId);}catch(e){recordError("MATCH INGESTION",e,{file:file.filename,match:matchLabel});}}}
  const matchIds=[...auditedMatchIds];setAuditProgress(0);
  ...
  try{
    ...
    if(runIds.length){const byRun=new Map<string,StageProgressRow[]>();for(const runId of runIds){const rows=await truthApi.getStages(runId);byRun.set(runId,rows as StageProgressRow[]);}setAuditProgress(computeBatchExecutionPercent(byRun));}
    ...
    toast.success(`${created} matches ingested, ${versions} summary versions, ${matchIds.length} audit runs queued`);
  }catch(error){
