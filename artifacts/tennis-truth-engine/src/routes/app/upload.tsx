...
import { ocrImageLocally, ocrPdfLocally } from "@/lib/pdf-ocr";
 ...
 const analyze=async()=>{
  ...
  try{
   ...
   for(const file of files){
    ...
    const isLarge=isPdf&&file.size>LARGE_PDF_SKIP_TEXT_EXTRACTION_BYTES;
    if(isLarge){
     toast.info(`${file.name} is ${(file.size/1024/1024).toFixed(1)}MB — likely a screenshot compilation, going straight to on-device OCR. Large batches (30+ matches) are more reliable split into a few smaller uploads.`);
    }else if(isPdf){
     try{pages=(await extractPdfText(file)).pages;matchups=parseSummaryText(pages);}
     catch(e){pages=[];recordError("PDF TEXT EXTRACTION",e,{file:file.name});}
    }
    if(matchups.length===0){
     setProgress(`${file.name}: running free local OCR…`);
     try{
      const progress=(m:string)=>setProgress(`${file.name}: ${m}`);
      const ocr=isPdf?await ocrPdfLocally(file,progress):await ocrImageLocally(file,progress);
      pages=ocr.pages;matchups=parseSummaryText(pages);source="LOCAL_OCR";
     }catch(e){recordError("LOCAL OCR",e,{file:file.name});}
    }
    if(matchups.length===0&&isPdf&&!isLarge){
     setProgress(`${file.name}: local OCR found no matchup — trying vision extraction…`);
     try{const base64=await toBase64(file);const{matchups:ai}=await visionExtract({data:{filename:file.name,base64}});matchups=ai.map(aiToParsed);source="VISION";}
     catch(e){const message=recordError("VISION EXTRACTION",e,{file:file.name});if(/402|credit/i.test(message))toast.error(`${file.name}: local OCR could not identify the matchup and AI credits are exhausted.`);else toast.error(`${file.name}: ${message}`);}
    }
    matchups=dedupeMatchups(matchups);
    if(!matchups.length){recordError("MATCHUP DETECTION","No matchups detected — review required",{file:file.name});toast.warning(`${file.name}: no matchups detected — review required`);}
   ...
   setStaged(s=>[...s,...next]);
   setFiles([]);
  }catch(e){const message=recordError("START ANALYSIS",e);toast.error(`Analysis failed: ${message}`);}
