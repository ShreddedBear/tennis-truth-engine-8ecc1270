...
function Slate(){
  ...
  const{data}=useQuery({
    ...
    queryFn:async()=>{
      const response=await truthApi.getSlate();
      const raw=response.matches as any[],runRows=response.runs as any[],versions=response.versions as any[],groups:any[][]=[];
      ...
      const coverage=response.coverage as any[];
