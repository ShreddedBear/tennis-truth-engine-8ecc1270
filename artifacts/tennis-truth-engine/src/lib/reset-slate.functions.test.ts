...
function fakeApi(
  ...
  const clearTruthSlate = vi.fn(async () => {
    if (error) throw new Error(error);
    if (!result) throw new Error("clear slate returned no result");
    return result;
  });
  return { clearTruthSlate };
}
...
describe("clearOperationalSlate", () => {
  it("A/D. calls the single authoritative RPC, scoped to this app's operational owner", async () => {
    const api = fakeApi({ before: POPULATED_BEFORE, after: CLEAN_AFTER, deletedMatches: 50, deletedUploads: 3, deletedSlates: 1, deletedCalibrationObservations: 0 });
    await clearOperationalSlate(api);
    expect(api.clearTruthSlate).toHaveBeenCalledOnce();
    expect(LOCAL_WORKSPACE_ID).toBeTypeOf("string");
  });

  it("reports the real deleted counts, not a soft-clear count", async () => {
    const result = await clearOperationalSlate(fakeApi({ before: POPULATED_BEFORE, after: CLEAN_AFTER, deletedMatches: 50, deletedUploads: 3, deletedSlates: 1, deletedCalibrationObservations: 0 }));
  ...
  it("P. zero matches on the slate succeeds cleanly, reporting zero deletions", async () => {
    ...
    const result = await clearOperationalSlate(fakeApi({ before: emptyBefore, after: CLEAN_AFTER, deletedMatches: 0, deletedUploads: 0, deletedSlates: 0, deletedCalibrationObservations: 0 }));
  ...
  it("O. calling it twice in a row is safe: the second call reports nothing left to delete", async () => {
    await expect(clearOperationalSlate(fakeApi({ before: CLEAN_AFTER, after: CLEAN_AFTER, deletedMatches: 0, deletedUploads: 0, deletedSlates: 0, deletedCalibrationObservations: 0 }))).resolves.toMatchObject({ matches: 0 });
  });
  ...
  it("refuses to report success if the database's own AFTER snapshot shows survivors", async () => {
    ...
    await expect(clearOperationalSlate(fakeApi({ before: POPULATED_BEFORE, after: { ...CLEAN_AFTER, matches: 50 }, deletedMatches: 0, deletedUploads: 0, deletedSlates: 0, deletedCalibrationObservations: 0 }))).rejects.toThrow(/did not fully delete/i);
  });

  it("surfaces a database error rather than reporting a silent, incomplete success", async () => {
    await expect(clearOperationalSlate(fakeApi(null, "permission denied for authenticated clear-slate API"))).rejects.toThrow(/permission denied/);
  });
});
