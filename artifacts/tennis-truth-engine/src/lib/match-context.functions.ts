...
async function persistedContext(p1:string,p2:string,hints:Fields):Promise<{fields:Fields;sources:string[];provenance:MetadataProvenanceMap;tour:Tour|null}> {
  try {
    const { rows } = await truthServerOperation<{ rows: Array<{player1_name:string;player2_name:string;tournament_name:string|null;event_level:string|null;round:string|null;scheduled_date:string|null;surface:string|null;best_of:number|null;updated_at:string|null}> }>("match-context");
