...
import { isExecutableMetricCode, normalizeMetricCode } from "./truth-engine-active-metrics";
...
function unavailableReason(message:string):UnavailableReason{const v=message.toLowerCase();if(v.includes("timeout")||v.includes("timed out"))return"PROVIDER_TIMEOUT";if(v.includes("rate limit")||v.includes("429"))return"API_RATE_LIMIT";if(v.includes("auth")||v.includes("api key")||v.includes("401")||v.includes("403"))return"PROVIDER_AUTH_FAILED";if(v.includes("player")&&v.includes("not found"))return"PLAYER_NOT_FOUND";if(v.includes("match")&&v.includes("not found"))return"MATCH_NOT_FOUND";if(v.includes("surface"))return"SURFACE_DATA_NOT_FOUND";if(v.includes("sample"))return"INSUFFICIENT_SAMPLE";if(v.includes("missing")||v.includes("required input"))return"MISSING_REQUIRED_INPUT";if(v.includes("conflict"))return"SOURCE_CONFLICT";
// \bpars, not includes("parse"): a plain substring check matches "parse" inside "sparse" --
// several real reason strings say a lane structurally lacks data "...not sparse data" (see
// audit-metric-027/029/045/052's laneOutcome NOT_ENOUGH_DATA messages), and that word alone
// was enough to misclassify a genuine structural-absence explanation as PARSING_FAILED. The
// word boundary requires "pars" to start a word (parse/parsing/parsed/parser), which
// "sparse" never does.
if(/\bpars/.test(v)||v.includes("json"))return"PARSING_FAILED";
if(v.includes("historical"))return"HISTORICAL_DATA_UNAVAILABLE";return"NO_SOURCE_FOUND";}
...
// The execution universe is intentionally wider than the graded/voting universe:
// isExecutableMetricCode includes ACTIVE_METRIC_CODES plus a narrow persistence-only
// set of implemented producers. Persistence-only rows execute and retain their real
// findings, but cannot affect readiness, comparison, family voting, or decisions until
// a measured ComparisonSpec promotes them into ACTIVE_METRIC_CODES.
...
const activeMetricDefs=metricDefs.filter(d=>isExecutableMetricCode(d.rule_code)||isProcessMetaRuleCode(d.rule_code)||normalizeMetricCode(d.rule_code)==="042");
