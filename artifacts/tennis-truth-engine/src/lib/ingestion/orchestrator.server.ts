...
import { ingestionFailRun, ingestionFinishRun, ingestionStartRun } from "../truth-server-api";
...
async function runTracked<T extends IngestionResult>(sourceId: SourceId, jobType: string, fn: () => Promise<T>) {
  const { run } = await ingestionStartRun(sourceId, jobType);
  try {
    const result = await fn();
    assertMeaningfulIngestion(sourceId, result);
    const written = Number(result.observations_written ?? 0);
    await ingestionFinishRun(run.id, { recordsSeen: written, recordsInserted: written, metadata: result });
    return result;
  } catch (err) {
    await ingestionFailRun(run.id, ingestionErrorMessage(err));
    throw err;
  }
}
