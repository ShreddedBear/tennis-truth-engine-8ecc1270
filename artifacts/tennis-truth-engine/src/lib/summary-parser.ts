...
function normalizedSurface(value:string):{surface:string;indoor:string}|null{const token=cleanLine(value).replace(/[-_]/g," ").toLowerCase();if(/\bindoor\s*hard\b|\bcarpet\b/.test(token))return{surface:"IndoorHard",indoor:"Indoor"};if(/\b(?:red\s+)?clay\b/.test(token))return{surface:"Clay",indoor:"Outdoor"};if(/\bgrass\b/.test(token))return{surface:"Grass",indoor:"Outdoor"};if(/\bhard\b/.test(token))return{surface:"Hard",indoor:"Outdoor"};return null;}
function addSurface(out:ParsedField[],value:string,page:number,confidence=.95){const normalized=normalizedSurface(value);if(!normalized)return;add(out,"surface",normalized.surface,page,confidence);add(out,"indoor_outdoor",normalized.indoor,page,confidence);}
function fieldsFromBlock(block:string,page:number){const out:ParsedField[]=[];for(const[key,re]of FIELD_PATTERNS){const m=block.match(re);if(!m)continue;const raw=(m[1]??"").trim().replace(/\s{2,}.*$/,"");if(raw)add(out,key,key==="tournament"?cleanTournament(raw):raw,page);}
 ...
 const surfaceField=out.find(f=>f.field_key==="surface");if(surfaceField?.normalized_value){const normalized=normalizedSurface(surfaceField.normalized_value);if(normalized){surfaceField.normalized_value=normalized.surface;add(out,"indoor_outdoor",normalized.indoor,page,surfaceField.confidence);}}
 return out;}
...
function parseFixtureTable(pageText:string,page:number):ParsedMatchup[]{const out:ParsedMatchup[]=[];const lines=pageText.replace(/\r/g,"").split("\n");let tournament:string|null=null,eventLevel:string|null=null,surface:string|null=null,indoor:string|null=null,round:string|null=null,bestOf:string|null=null;for(let i=0;i<lines.length;i++){const line=cleanLine(lines[i]);if(/^(?:ATP|WTA|ITF)\b/i.test(line)&&!/\b(?:PLAYER|EVENT DATA)\b/i.test(line)){tournament=line;eventLevel=/challenger/i.test(line)?"Challenger":/^ATP\b/i.test(line)?"ATP":/^WTA\b/i.test(line)?"WTA":"ITF";surface=indoor=round=bestOf=null;continue;}const metadata=line.match(/^(Indoor\s+Hard|Hard|Clay|Red\s+Clay|Grass|Carpet)\s*[•·|]\s*(Round[^•·|]+)\s*[•·|]\s*Best\s+of\s+([35])$/i);if(metadata){const normalized=normalizedSurface(metadata[1]);surface=normalized?.surface??null;indoor=normalized?.indoor??null;round=cleanLine(metadata[2]);bestOf=metadata[3];continue;}const columns=lines[i].trim().split(/\s{2,}/);if(!tournament||columns.length<3||! /^\d+$/.test(columns[0]))continue;const p1=cleanPlayer(columns[1]),p2=cleanPlayer(columns[2]);if(!looksLikeFullPlayerName(p1)||!looksLikeFullPlayerName(p2))continue;const fields:ParsedField[]=[];add(fields,"tournament",tournament,page,.99);if(eventLevel)add(fields,"event_level",eventLevel,page,.98);if(round)add(fields,"round",round,page,.98);if(bestOf)add(fields,"best_of",bestOf,page,.98);if(surface){add(fields,"surface",surface,page,.99);if(indoor)add(fields,"indoor_outdoor",indoor,page,.99);}out.push({player1_name:p1,player2_name:p2,page_number:page,fields,confidence:.99});}return out;}
export function parseSummaryText(pages:string[]){const matchups:ParsedMatchup[]=[];pages.forEach((pageText,idx)=>{const page=idx+1;const tableMatchups=parseFixtureTable(pageText,page);if(tableMatchups.length){matchups.push(...tableMatchups);return;}const lines=pageText.split(/\n/).map(cleanLine).filter(Boolean);const anchors:number[]=[];lines.forEach((l,i)=>{if(VS.test(l))anchors.push(i);});if(anchors.length){anchors.forEach((anchorIdx,k)=>{const nextAnchor=anchors[k+1]??lines.length;const block=lines.slice(anchorIdx,nextAnchor).join("\n");const m=lines[anchorIdx].match(VS);if(!m)return;const[p1,p2]=canonicalNamesAroundAnchor(lines,anchorIdx,m[1]??"",m[2]??"");if(!p1||!p2)return;const fields=fieldsFromBlock(block,page);matchups.push({player1_name:p1,player2_name:p2,page_number:page,fields,confidence:Number(Math.min(1,.72+fields.length*.03).toFixed(2))});});return;}const inferred=inferPairWithoutVs(lines);if(!inferred)return;const fields=fieldsFromBlock(lines.join("\n"),page);matchups.push({player1_name:inferred[0],player2_name:inferred[1],page_number:page,fields,confidence:Number(Math.min(.95,.75+fields.length*.03).toFixed(2))});});return matchups;}
