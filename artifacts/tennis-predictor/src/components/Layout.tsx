...
const MOBILE_MORE_ADMIN: NavItem[] = [
  { href: "/admin/users", label: "Users & Subs", icon: Users, exact: false },
    ...
    <div className="min-h-[100dvh] flex flex-col bg-background text-foreground font-sans overflow-x-hidden selection:bg-primary/20 selection:text-primary">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border/70 bg-background/90 backdrop-blur-xl supports-[backdrop-filter]:bg-background/75 shadow-sm shadow-black/30">
        <div className="app-container min-h-[3.75rem] py-2.5 flex items-center justify-between gap-2">
          ...
          <nav className="hidden md:flex items-center gap-5 text-[0.8125rem] font-medium">
            {NAV_LINKS.map(({ href, label, exact, external }) => {
              const active = isActive(href, location, exact)
              if (external) {
                return (
                  <a
                    key={href}
                    href={href}
                    className="relative py-1 transition-all text-muted-foreground hover:text-primary"
                  >
                    {label}
                  </a>
                )
            ...
            {isAdmin && (
              <>
                ...
                {ADMIN_NAV_LINKS.map(({ href, label, exact, external }) => {
                  const active = isActive(href, location, exact)
                  if (external) {
                    return (
                      <a
                        key={href}
                        href={href}
                        className="relative py-1 transition-all text-amber-500/60 hover:text-amber-400"
                        title={`Admin: ${label}`}
                      >
                        {label}
                      </a>
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-border/50 bg-background/95 backdrop-blur-xl">
            <nav className="app-container py-3 flex flex-col gap-0.5">
              ...
              {NAV_LINKS.map(({ href, label, icon: Icon, exact, external }) =>
                external ? (
                  <a
                    key={href}
                    href={href}
                    onClick={() => setMobileOpen(false)}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all text-muted-foreground hover:bg-secondary hover:text-foreground"
                  >
                    <Icon className="w-4 h-4 shrink-0" />
                    {label}
                  </a>
              ...
              {isAdmin && (
                <>
                  ...
                  {ADMIN_NAV_LINKS.map(({ href, label, icon: Icon, exact, external }) => {
                    const active = isActive(href, location, exact)
                    if (external) {
                      return (
                        <a
                          ...
                          href={href}
                          onClick={() => setMobileOpen(false)}
                          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all text-amber-500/60 hover:bg-amber-500/10 hover:text-amber-400"
                        >
                          <Icon className="w-4 h-4 shrink-0" />
                          {label}
                        </a>
      ...
      {mobileMoreOpen && (
        <div className="md:hidden fixed bottom-[4.25rem] left-3 right-3 z-50 rounded-2xl border border-border/70 bg-background shadow-xl p-2">
          {/* Subscriber items */}
          {MOBILE_MORE_SUBSCRIBER.map(({ href, label, icon: Icon, exact, external }) => {
            const active = isActive(href, location, exact)
            if (external) {
              return (
                <a
          ...
          {isAdmin && (
            <>
              ...
              {MOBILE_MORE_ADMIN.map(({ href, label, icon: Icon, exact, external }) => {
                const active = isActive(href, location, exact)
                if (external) {
                  return (
                    <a
                      ...
                      href={href}
                      onClick={() => setMobileMoreOpen(false)}
                      className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium text-amber-500/60 hover:bg-amber-500/10 hover:text-amber-400"
                    >
                      <Icon className="w-4 h-4 shrink-0" />
                      {label}
                    </a>
