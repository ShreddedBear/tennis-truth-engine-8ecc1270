...
START_YEAR=2025; CURRENT_YEAR=datetime.now(timezone.utc).year; MAX_PAGES=100
...
def get(path,params=None):
    ...
    try:
        ...
        return e.code,b
    except Exception as e:return 0,{'error':type(e).__name__,'message':str(e)}
...
def main():
    OUT.mkdir(parents=True,exist_ok=True); by_id,by_hash=load_existing(); append=[]
    counters={'examined':0,'wta_challenger_matches':0,'pbp_confirmed_this_scan':0,'pbp_missing':0,'duplicates_already_stored':0,'hash_collisions':0,'http_failures':0,'new_records':0}; years={}
    for year in range(CURRENT_YEAR,START_YEAR-1,-1):
        yc={'examined':0,'wta_challenger_matches':0,'pbp_confirmed_this_scan':0,'pbp_missing':0,'already_stored':0,'new_records':0}
        for page in range(MAX_PAGES):
            st,data=get('/matches/',{'status':'finished','date_from':f'{year}-01-01','date_to':f'{year}-12-31','limit':200,'offset':page*200})
            if st!=200:counters['http_failures']+=1;break
            ...
            for m in matches:
                ...
                if ps!=200 or not pbp_meaningful(payload):
                    if ps!=200:counters['http_failures']+=1
                    yc['pbp_missing']+=1;counters['pbp_missing']+=1;continue
                yc['pbp_confirmed_this_scan']+=1;counters['pbp_confirmed_this_scan']+=1
                blob=json.dumps(payload,sort_keys=True,separators=(',',':'));sha=hashlib.sha256(blob.encode()).hexdigest()
                if sha in by_hash:counters['hash_collisions']+=1;continue
                ...
                append.append(rec);by_id[str(mid)]=rec;by_hash[sha]=rec;counters['new_records']+=1;yc['new_records']+=1;time.sleep(.08)
        years[str(year)]=yc;print('YEAR_UPDATE '+json.dumps({'year':year,**yc},separators=(',',':')),flush=True)
    if append:
        with RAW.open('a') as f:
            for r in append:f.write(json.dumps(r,separators=(',',':'))+'\n')
    stored_by_year=Counter(str(r.get('year')) for r in by_id.values())
    state={'scope':'WTA Challenger/WTA 125 only','source':'BSD','start_year':START_YEAR,'through_year':CURRENT_YEAR,'updated_at_utc':datetime.now(timezone.utc).isoformat(),'totals':counters,'years':years,'stored_pbp_by_year':dict(stored_by_year),'unique_stored_match_ids':len(by_id),'unique_stored_pbp_hashes':len(by_hash),'firewall':{'persisted_pbp_revalidated':True,'match_id_unique':True,'pbp_hash_unique':True,'other_tours_excluded':True}}
    STATE.write_text(json.dumps(state,indent=2)+'\n')
    lines=['# BSD WTA Challenger PBP','', '- Scope: **WTA Challenger/WTA 125 only**',f'- Years: **{START_YEAR}–{CURRENT_YEAR}**',f'- Unique stored matches with usable PBP: **{len(by_id)}**',f'- Unique stored PBP hashes: **{len(by_hash)}**',f"- PBP-confirmed matches encountered this scan (including already stored): **{counters['pbp_confirmed_this_scan']}**",f"- New records this run: **{counters['new_records']}**",f"- Missing/no usable PBP this run: **{counters['pbp_missing']}**",f"- Already-stored match IDs safely skipped: **{counters['duplicates_already_stored']}**",f"- Duplicate PBP hashes blocked: **{counters['hash_collisions']}**",'', '| Year | WTA Challenger matches | PBP confirmed | Already stored | PBP missing | New stored | Total stored PBP |','|---:|---:|---:|---:|---:|---:|---:|']
    for y in sorted(years,reverse=True):
        x=years[y];lines.append(f"| {y} | {x['wta_challenger_matches']} | {x['pbp_confirmed_this_scan']} | {x['already_stored']} | {x['pbp_missing']} | {x['new_records']} | {stored_by_year.get(y,0)} |")
