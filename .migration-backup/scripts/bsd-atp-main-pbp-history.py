...
HEAD={'Authorization':f'Token {TOKEN}','User-Agent':'tennis-truth-engine-bsd-atp-history/1.0'}
OUT=Path('data/audit/bsd-atp-main-pbp-history')
PAGE_SIZE=200
MAX_PAGES=100
...
def atp_main(m):
    """Strict ATP-main classifier. Never infer ATP merely from male gender."""
    ...
    if circuit != 'ATP':
        return False
    exclusions=('challenger','utr','itf','futures','satellite','exhibition')
...
def year_scan(year):
    ...
    summary={
        'year':year,'matches_listed_all_tours':listed,'atp_main_matches':classified,
        'atp_main_pbp_requests':len(rows),'pbp_available':len(good),
    ...
    (d/'UPDATE.md').write_text(
        f"* {classified:,} strict ATP Main finished matches examined for {year}.\n"
        f"* {len(good):,} BSD matches returned usable point-by-point data.\n"
        f"* {len(rows)-len(good):,} returned no usable PBP.\n"
        f"* Earliest confirmed BSD ATP Main PBP date in {year}: {summary['earliest_confirmed_pbp_date'] or 'NONE'}.\n"
