...
**How to apply:** Any OCR provider, resolver, cache, or parser change must keep both deadlines, preserve the batch historical fast path, bound optional fixture/tournament context, avoid caching transient degraded resolution, and test both a valid matchup image/text path and a non-matchup UI screen.