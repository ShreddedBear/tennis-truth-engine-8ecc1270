//#region node_modules/.nitro/vite/services/ssr/assets/meta-derived-evidence.server-COP3pmmu.js
var META_CODES = [
	"048",
	"049",
	"056",
...
var META_SET = new Set(META_CODES);
var STRESS_META_CODES = ["050", "058"];
...
function uniqueSources(rows) {
	const out = [];
	for (const row of rows) for (const source of sourceRefs(row)) if (!out.some((x) => x.source_name === source.source_name && x.url === source.url)) out.push(source);
	return out;
}
function explicitFamily(row) {
...
function sideStats(rows, side) {
	...
	const usable = rows.filter((row) => {
		const treatment = String(row[treatmentKey] ?? "");
		return USABLE.has(treatment) && row[valueKey] !== null && row[valueKey] !== void 0 && row[valueKey] !== "";
	});
	const unavailable = rows.filter((row) => String(row[treatmentKey] ?? "") === "UNAVAILABLE").length;
	const families = usable.map(explicitFamily).filter((x) => !!x);
	...
	const samplesReported = usable.filter((row) => row["sample"] !== null && row["sample"] !== void 0 && String(row["sample"]).trim() !== "").length;
...
function valuesFor(stats, total) {
	...
	return {
		...
		"049": stats.distinctFamilies === 0 ? {
			...
			reliability: null,
			missing: ["at least one explicitly assigned evidence family"]
		} : {
		...
		"057": noUsable ? {
			...
			treatment: "UNAVAILABLE",
			reliability: null,
			missing: ["at least one usable sourced metric"]
		} : {
...
*/
async function applySafeMetaDerivedMetrics(deps, runId) {
	const rows = await deps.list("metric_results", runId);
	const base = rows.filter((row) => !META_SET.has(codeOf(row)) && !STRESS_META_CODES.includes(codeOf(row)) && row["matrix_derived"] !== true);
	...
	const matchFamilies = /* @__PURE__ */ new Set([...p1Stats.explicitFamilies, ...p2Stats.explicitFamilies]);
	let changed = false;
	for (const code of META_CODES) {
		const row = targets.get(code);
		if (!row) continue;
		const a = p1[code], b = p2[code];
		if (String(row["p1_treatment"] ?? "") === a.treatment && String(row["p2_treatment"] ?? "") === b.treatment && (row["p1_value"] ?? null) === a.value && (row["p2_value"] ?? null) === b.value && row["evidence_family"] === null) continue;
		const now = deps.now().toISOString();
		const partial = a.treatment === "PARTIAL" || b.treatment === "PARTIAL";
		const missing = [.../* @__PURE__ */ new Set([...a.missing, ...b.missing])];
		const reliabilityValues = [a.reliability, b.reliability].filter((x) => x !== null);
...
*/
async function applySafeStressDerivedMetrics(deps, runId) {
	...
	if (!completed.length) {
		...
		for (const code of STRESS_META_CODES) {
			...
			await deps.update("metric_results", String(row["id"]), {
				...
				unavailable_detail: "No completed stress/removal results.",
				missing_inputs: ["completed stress/removal tests"],
	...
	const countOutcome = (name) => completed.filter((row) => String(row["outcome"] ?? "").toUpperCase() === name).length;
	...
	const summaries = {
		"050": `completed_tests=${completed.length}/${stress.length}; stable=${stable}; mostly_stable=${mostly}; unstable=${unstable}; fails=${fails}; matrix_removal_survived=${matrixRemovalSurvived}; strongest_family_removal_survived=${familyRemovalSurvived}`,
		"058": `scenario_tests_completed=${completed.length}/${stress.length}; stable_or_mostly_pct=${stableRate.toFixed(1)}; stable=${stable}; mostly_stable=${mostly}; unstable=${unstable}; fails=${fails}`
	};
	let changed = false;
	for (const code of STRESS_META_CODES) {
		const row = targets.get(code);
		...
		if (row["p1_value"] === value && row["p2_value"] === value && String(row["p1_treatment"]) === "PARTIAL" && String(row["p2_treatment"]) === "PARTIAL" && row["evidence_family"] === null) continue;
		const now = deps.now().toISOString();
		const missing = code === "050" ? ["full leave-one-input-out and parameter perturbation universe beyond the persisted stress suite"] : ["full probability-distribution sensitivity and every scenario defined by the broad metric family"];
		await deps.update("metric_results", String(row["id"]), {
			p1_value: value,
			p2_value: value,
			...
			p1_unavailable_reason: "MISSING_REQUIRED_INPUT",
			p2_unavailable_reason: "MISSING_REQUIRED_INPUT",
			unavailable_detail: `Partial robustness coverage; missing ${missing.join("; ")}.`,
			missing_inputs: missing
		});
		changed = true;
	}
...
//#endregion
export { applySafeMetaDerivedMetrics, applySafeStressDerivedMetrics };
