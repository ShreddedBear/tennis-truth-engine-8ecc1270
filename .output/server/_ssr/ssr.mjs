...
var DISK_PATH = join(process.cwd(), "data", "generated", "tennis-runtime-index.json");
var ASSET_PATH = "/generated/tennis-runtime-index.json.gz";
var cache = null;
function looksLikeLfsPointer(text) {
	return text.startsWith("version https://git-lfs.github.com/spec/v1");
}
function loadFromDisk() {
	try {
		const text = readFileSync(DISK_PATH, "utf8");
		if (!looksLikeLfsPointer(text)) return JSON.parse(text);
	} catch {}
	try {
...
async function ensureRuntimeIndexLoaded(assets) {
	if (cache) return;
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-KPZuT5q2.mjs").then((n) => n.t).then((m) => m.default ?? m);
	return serverEntryPromise;
}
function isServerFunctionRequest(request) {
...
async function normalizeCatastrophicSsrResponse(request, response) {
	...
	if (isServerFunctionRequest(request)) return new Response(JSON.stringify({
		ok: false,
		error: {
			code: "SERVER_FUNCTION_FAILED",
			message: "The audit request failed before returning a valid result. Persisted progress is safe and can be resumed."
		}
	}), {
		status: 500,
...
var server_default = { async fetch(request, env, ctx) {
	try {
		await ensureRuntimeIndexLoaded(env?.ASSETS);
...
//#endregion
export { server_default as default, renderErrorPage as n, loadRuntimeIndex as t };
