//#region node_modules/.nitro/vite/services/ssr/assets/opponent-win-pathways-meta.server-DhsbNQu1.js
function codeOf(row) {
...
function uniqueSources(rows) {
	const out = [];
	for (const row of rows) for (const source of sourcesOf(row)) if (!out.some((x) => x.source_name === source.source_name && x.url === source.url)) out.push(source);
	return out;
}
var METRIC_042_CODES = /* @__PURE__ */ new Set([
	"SERVE_THROUGH",
	"RETURN_PRESSURE",
	"TIEBREAK",
	"LONG_RALLY",
...
*/
async function applyOpponentWinPathwaysMetric(deps, runId, player1, player2) {
	const [metrics, pathways] = await Promise.all([deps.list("metric_results", runId), deps.list("underdog_results", runId)]);
	const target = metrics.find((row) => codeOf(row) === "042");
	if (!target) return false;
	const completed = pathways.filter((row) => {
		const code = String(row["pathway_code"] ?? "");
		return String(row["status"] ?? "") === "COMPLETE" && METRIC_042_CODES.has(code) && [
			"WEAK",
			"REALISTIC",
			"STRONG"
	...
	const summarizeOpponent = (opponent) => {
		const rows = completed.filter((row) => String(row["player_side"] ?? "") === opponent);
		if (!rows.length) return null;
		const classifications = rows.map((row) => {
			const family = pathwayFamily(String(row["pathway_code"] ?? ""));
			return family ? `${family}:${String(row["classification"] ?? "")}` : null;
		}).filter((x) => !!x);
		const realisticFamilies = new Set(rows.filter((row) => ["REALISTIC", "STRONG"].includes(String(row["classification"] ?? ""))).map((row) => pathwayFamily(String(row["pathway_code"] ?? ""))).filter((x) => !!x));
		return {
	...
	const p1Value = p1Summary ? `opponent=${player2}; pathway_classifications=${p1Summary.text}; realistic_pathways_count=${p1Summary.realisticCount}` : null;
	const p2Value = p2Summary ? `opponent=${player1}; pathway_classifications=${p2Summary.text}; realistic_pathways_count=${p2Summary.realisticCount}` : null;
	...
	if (String(target["p1_treatment"] ?? "") === p1Treatment && String(target["p2_treatment"] ?? "") === p2Treatment && (target["p1_value"] ?? null) === p1Value && (target["p2_value"] ?? null) === p2Value && target["evidence_family"] === null) return false;
	const sources = uniqueSources(completed.filter((row) => String(row["player_side"] ?? "") === player1 || String(row["player_side"] ?? "") === player2));
	...
	await deps.update("metric_results", String(target["id"]), {
		...
		reconstruction_reason: "Derived only from completed Dangerous Underdog classifications that correspond to metric 042's defined pathways. Extra audit-only pathway categories are excluded; calibrated probabilities are not stored.",
		...
		unavailable_detail: "PARTIAL only: pathway classifications and realistic-pathway count are supported, but metric 042 calls for calibrated pathway probabilities; none are fabricated.",
		missing_inputs: ["calibrated probability for each metric-042 opponent win pathway"]
	});
	return true;
}
//#endregion
export { applyOpponentWinPathwaysMetric };
