import { t as STAT_BY_KEY } from "./stat-catalog-qDs35q_a.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/engine-tr9AM3Mg.js
var pct = (num, den) => den > 0 ? num / den * 100 : null;
var RECONSTRUCTION_SPECS = [
	{
		id: "RS-SRV-01",
	...
	},
	{
		...
		formula: "first_serve_in_pct/100 × first_serve_points_won_pct + (1 − first_serve_in_pct/100) × second_serve_points_won_pct",
		required: [
			"first_serve_in_pct",
	...
	},
	{
		...
		minSample: {
			input: "break_points_opportunities",
			min: 20
		},
	...
	},
	{
		...
		compute: (v) => pct(v["first_serve_points_won"] + v["second_serve_points_won"] + v["return_points_won"], v["service_points"] + v["return_points"]),
		contextSensitive: true
	},
	{
		id: "RS-CMB-03",
	...
	},
	{
		...
		prohibited: [
			"Matrix Elo",
			"Matrix win probability",
			"any Matrix model output"
		],
		contextSensitive: true
	},
	{
...
for (const spec of RECONSTRUCTION_SPECS) for (const key of [
	spec.output,
	...spec.required,
	...spec.optional ?? []
]) if (!STAT_BY_KEY.has(key)) throw new Error(`Reconstruction spec ${spec.id} references unknown stat "${key}"`);
RECONSTRUCTION_SPECS.reduce((acc, s) => {
	(acc[s.output] ??= []).push(s);
	return acc;
}, {});
new Set(RECONSTRUCTION_SPECS.map((s) => STAT_BY_KEY.get(s.output).family));
var norm = (v) => v ? v.trim().toLowerCase() : null;
var ctxKey = (s) => `${norm(s.player)}|${norm(s.surface) ?? "*"}|${norm(s.window) ?? "*"}`;
...
function sanitizeEvidence(raw) {
	...
	for (const s of raw) {
		...
		if (!s.sources?.length || !s.sources.some((src) => src.source_name?.trim())) continue;
		out.push({
			...s,
			origin: "DIRECT",
...
function describe(spec, values, result) {
...
*/
function reconstruct(evidenceIn) {
	const evidence = sanitizeEvidence(evidenceIn);
	const table = /* @__PURE__ */ new Map();
	const put = (s) => {
		const ctx = ctxKey(s);
		const bucket = table.get(ctx) ?? /* @__PURE__ */ new Map();
	...
	for (let pass = 0; pass < 4; pass++) {
		...
		for (const [, bucket] of table) {
			...
			for (const spec of RECONSTRUCTION_SPECS) {
				...
				if (spec.minSample) {
					const observed = bucket.get(spec.minSample.input)?.value ?? inputs.find((i) => i.sample != null)?.sample ?? null;
				...
				if (result === null || !Number.isFinite(result) || result < def.min || result > def.max) {
					blockedMap.set(blockKey, {
						...
						window: any.window,
						missing: [],
						reason: result === null ? "Formula is undefined for the sourced inputs (zero denominator)." : `Computed value ${round(result)} falls outside the admissible range for ${spec.output}; discarded rather than clamped.`
					});
					continue;
				}
				const stat = {
	...
	return {
		derived,
		blocked: [...blockedMap.values()].filter((b) => !succeeded.has(`${b.spec_id}|${norm(b.player)}|${norm(b.surface) ?? "*"}|${norm(b.window) ?? "*"}`))
	};
}
...
//#endregion
export { reconstruct as n, sanitizeEvidence as r, RECONSTRUCTION_SPECS as t };
