...
describe("sequential certification guardrails for 034/036/038/039/040", () => {
  ...
  it("prevents generic atomic pass-2 statistics from cross-wiring into composite families", () => {
    ...
    for (const key of ["total_points_won_pct","dominance_ratio","break_points_opportunities","hold_pct","break_pct","ace_rate_pct","double_fault_rate_pct","first_serve_points_won_pct","second_serve_points_won_pct","return_points_won_pct","avg_match_minutes","deciding_set_win_pct"]) expect(PROTECTED.has(String(familyOf(key))), `${key} must remain an input, never a substitute for 034/036/038/039/040`).toBe(false);
  });
  ...
  it("keeps protected metrics out of local historical and summary fallback maps", () => {
    ...
    for (const code of PROTECTED) { expect(sweep).not.toContain(`  "${code}":[`); expect(hybrid).not.toContain(`"${code}":[`); }
  });
  ...
  it("downgrades incomplete exact composite RECONSTRUCTED evidence to PARTIAL without counting raw neighbors", () => {
    const finding = validateMetric({ code:"034", name:"Scoreline Deception Index", body:null }, {
      metric_code:"034", p1_value:"scoreline vs point dominance=+1.2; scoreline vs dominance ratio=+0.08", p2_value:"scoreline vs point dominance=-1.2; scoreline vs dominance ratio=-0.08",
  ...
  it("rejects proxy-only PARTIAL evidence for protected composites", () => {
    const finding = validateMetric({ code:"040", name:"Hidden Decline Detector", body:null }, { metric_code:"040", p1_value:"ranking trend down 12 places", p2_value:"recent win pct=40", p1_treatment:"PARTIAL", p2_treatment:"PARTIAL", differential:null, evidence_family:"FORM", reliability:.8, sample:"10", unavailable_reason:null, sources:src });
    expect(finding.p1_treatment).toBe("UNAVAILABLE"); expect(finding.p2_treatment).toBe("UNAVAILABLE"); expect(finding.p1_value).toBeNull(); expect(finding.p2_value).toBeNull();
  });

  it("requires opponent-norm residual components for 038 rather than raw own-player aggregates", () => {
    const finding = validateMetric({ code:"038", name:"Opponent-Adjusted Residual Performance", body:null }, { metric_code:"038", p1_value:"hold_pct=84; break_pct=22", p2_value:"hold_pct=79; break_pct=25", p1_treatment:"RECONSTRUCTED", p2_treatment:"RECONSTRUCTED", differential:null, evidence_family:"SERVE_RETURN", reliability:.9, sample:"20", unavailable_reason:null, sources:src });
    expect(finding.p1_treatment).toBe("UNAVAILABLE"); expect(finding.p2_treatment).toBe("UNAVAILABLE");
  });

  it("requires pre-match expectation and rolling last-10 semantics for full 039 reconstruction", () => {
    const finding = validateMetric({ code:"039", name:"Performance Surprise Rating", body:null }, { metric_code:"039", p1_value:"actual performance 61; expected performance 55; performance surprise +6", p2_value:"actual performance 49; expected performance 52; performance surprise -3", p1_treatment:"RECONSTRUCTED", p2_treatment:"RECONSTRUCTED", differential:null, evidence_family:"SURPRISE", reliability:.9, sample:"1", unavailable_reason:null, sources:src });
    expect(finding.p1_treatment).toBe("PARTIAL"); expect(finding.p2_treatment).toBe("PARTIAL"); expect(finding.missing_inputs).toContain("pre-match expected performance"); expect(finding.missing_inputs).toContain("rolling last-10 surprise");
  });
  ...
  it("does not count a persisted value as supported when provenance/treatment is unusable", () => {
    ...
    expect(rows.find(row => row.side === "P1")?.classification).toBe("MAPPING_OR_PROVENANCE"); expect(rows.find(row => row.side === "P2")?.classification).toBe("SUPPORTED");
  });
  ...
  it("forbids proxy-only PARTIAL and incomplete RECONSTRUCTED treatment at the research layer", () => {
    ...
    expect(researcher).toContain("PARTIAL means that some, but not all, of the exact definition's required inputs or observations are directly supported"); expect(researcher).toContain("A proxy alone is UNAVAILABLE for that metric, not PARTIAL"); expect(researcher).toContain("RECONSTRUCTED is allowed only when every required component of the exact formula/definition is sourced"); expect(researcher).toContain("Do not substitute a convenient statistic for the statistic the definition actually requires");
  });

  it("protects formula semantics for residual and surprise metrics", () => {
    const master = readFileSync("public/seed/metrics.txt", "utf8");
    expect(master).toContain("compares to how other\n  players typically hold against that same opponent"); expect(master).toContain("difference between a player's actual performance and their pre-match\n  expected performance, calculated per match"); expect(master).toContain("trend of match-level performance surprise across the last ten\n  matches");
  });
