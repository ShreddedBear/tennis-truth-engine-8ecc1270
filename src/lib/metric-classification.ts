...
const PROTECTED: ClassificationRecord[] = [
  ...
  {
    metric_code: "022",
    metric_name: "Serve/Return Shot-Level Efficiency",
    classification: "PROTECTED_UNAVAILABLE",
    required_raw_fields: "Charted serve+1/return+1 shot-level outcomes, rally-state labels, unreturned-serve and return-in-play detail, second-serve-attack shot outcomes -- all of this code's ~26 named bullets need shot-by-shot outcome, direction, depth, or rally-position data",
    sources_checked: [
      "approved BSD PBP adapters (bsd-atp-main-pbp.server.ts, bsd-wta-main-pbp.server.ts, bsd-atp-challenger-pbp.server.ts, bsd-wta-challenger-pbp.server.ts) — point winner and score state only, no shot-level outcome, direction, depth, or rally-position fields (same confirmed gap as codes 017/054/074)",
      "four-tour historical results (runtime-tennis-index) — match-level only",
      "source_observations table — no shot-tracking observation_type",
      "src/lib/metric-certification.ts's own registered policy for \"022\" (permittedRawInputs: charted serve+1/return+1 outcomes, rally-state labels, shot-level outcomes) — already correctly aligned to the real catalog, confirming no deterministic engine anywhere in this repository targets it (verified via repo-wide search for `\"022\"` across src/lib/*.server.ts; the only other references are this certification policy, evidence-gap.ts's own SOURCE_REQUIRED classification, and historical-results-recovery.ts's Task 20 comment noting it was removed with no retarget since its real definition needs shot-level data this repository does not have)",
      "protected-metric-wiring.server.ts live web-search LLM researcher (aiResearcher) -- checked and found not practically productive for this fact type: the required named components essentially never appear in generically web-indexed, LLM-searchable form for individual active pro players, so this pathway is logged as checked rather than silently omitted",
    ],
    reconstruction_attempted: true,
    reconstruction_result: "NO_QUALIFYING_FIELDS — approved PBP contains point winner and score state, not shot-level serve+1/return+1 outcomes, rally-state, or shot direction/depth",
    reason: "Every one of this code's named bullets (Serve +1 Effectiveness, Return +1 Effectiveness, Unreturned Serve Rate, Return-in-Play Rate, Second-Serve Attack Rate, Second-Serve Punishment Differential, First-Strike Win Rate, Neutral-Rally Win Rate, Defensive Conversion Rate, forehand/backhand damage rates, crosscourt/down-the-line efficiency, and the rest of this code's ~26-bullet definition) requires shot-by-shot outcome, direction, depth, or rally-position data. Approved PBP in this system is point/score-state only (server, point winner, game/set boundary) -- the same missing-data class already established for codes 017, 054, and 074. No shot-tracking data source is ingested anywhere in this system. Added during the 2026-08-29 sequential metric-audit series (docs/metric-audit-021-022-surface-environment-and-shot-level.md) after confirming, independently, the same conclusion evidence-work-blockers.md item 2 had already reached informally (\"not confirmed anywhere in the approved evidence universe anyway\") but had never been formalized into this canonical registry -- meaning code 022 was, until this pass, silently sitting in the player-evidence denominator as if it were still open/unaudited, permanently unfillable, without ever draining the denominator honestly the way its shot-level siblings already do.",
    whether_future_ingestion_could_change_status: true,
    date_classified: "2026-08-29",
    review_status: "REVIEWED",
  },
  {
    metric_code: "078",
