...
function classifyText(...values: unknown[]): RepresentativeId | null {
  return classifyEvidenceTourFamily(...values);
}
...
async function deterministic(metric:Metric,match:RepresentativeMatch){const runners=[()=>deterministicRankingMetric({metricCode:metric.code,p1:match.p1,p2:match.p2,asOfDate:match.date,context:match.context}),()=>deterministicRulesContextMetric({metricCode:metric.code,p1:match.p1,p2:match.p2,asOfDate:match.date,context:match.context}),()=>deterministicEnvironmentMetric({metricCode:metric.code,p1:match.p1,p2:match.p2,asOfDate:match.date,tournament:match.tournament}),()=>deterministicMarketMetric({metricCode:metric.code,p1:match.p1,p2:match.p2,asOfDate:match.date,tournament:match.tournament,context:match.context}),()=>deterministicResultsScheduleMetric({metricCode:metric.code,p1:match.p1,p2:match.p2,asOfDate:match.date,tournament:match.tournament,eventLevel:match.event_level,tourFamily:match.id,context:match.context})];const errors:string[]=[];for(const runner of runners){try{const row=await runner();if(row)return{row,errors};}catch(error){errors.push(error instanceof Error?error.message:String(error));}}return{row:null,errors};}
...
function sameCircuit(a:RepresentativeId,b:RepresentativeId){return(a.startsWith("ATP_")&&b.startsWith("ATP_"))||(a.startsWith("WTA_")&&b.startsWith("WTA_"));}
function storedContextCompatible(row:any,match:RepresentativeMatch){
 const expectedTournament=normalizeEvidenceTournament(match.tournament),actualTournament=normalizeEvidenceTournament(row.tournament);
 if(expectedTournament&&actualTournament&&expectedTournament!==actualTournament)return false;
 if(match.surface&&row.surface&&String(match.surface).toLowerCase()!==String(row.surface).toLowerCase())return false;
 let sources="";try{sources=JSON.stringify(row.sources??[]);}catch{}
 const family=classifyText(row.tournament,row.sample_label,row.evidence_family,sources);
 if(family)return row.evidence_family==="RANKING"?sameCircuit(match.id,family):family===match.id;
