...
type RepresentativeMatch = { id:RepresentativeId; match_id:string; p1:string; p2:string; date:string; date_source:"scheduled_date"|"parsed_summary"|"created_at"|"warehouse_event_date"|"verified_index_date"; tournament:string; context:string; event_level:string|null; surface:string|null; sampling_source:"matches"|"source_observations"|"matches_plus_rankings"|"verified_pbp_index" };
...
async function classifyFromExactRankingEvidence(row:MatchCandidate):Promise<"ATP_MAIN"|"WTA_MAIN"|null> {
  ...
  const identities=await resolveCanonicalEvidencePair(row.player1_name,row.player2_name);
...
async function representativeMatches():Promise<{matches:RepresentativeMatch[];missing_classes:RepresentativeId[];missing_class_reasons:Partial<Record<RepresentativeId,string>>}> {
  ...
  for(const id of missingClasses)missing_class_reasons[id]=`No real persisted ${id} match with sufficient tour context and no qualifying paired warehouse observation were available for diagnostic sampling.`;
...
export async function runEvidenceCoverageRuntimeDiagnostic(){const metrics=await activeMetrics();if(metrics.length!==81)throw new Error(`Expected 81 active metrics, found ${metrics.length}`);const sample=await representativeMatches();if(!sample.matches.length)throw new Error("No real persisted matches or paired warehouse observations were available for evidence coverage sampling");const matches:any[]=[];
  for(const sampled of sample.matches){
    ...
    const identityPromise=(match.sampling_source==="matches"||match.sampling_source==="matches_plus_rankings")?db.from("matches").select("id,player1_name,player2_name,event_level,scheduled_date,surface").eq("id",match.match_id).limit(1):Promise.resolve({data:[],error:null});
