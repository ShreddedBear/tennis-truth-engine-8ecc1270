...
async function representativeMatches():Promise<{matches:RepresentativeMatch[];missing_classes:RepresentativeId[];missing_class_reasons:Partial<Record<RepresentativeId,string>>;class_proof:Partial<Record<RepresentativeId,unknown>>}> {
  ...
  for(const id of missingClasses)missing_class_reasons[id]=`No real persisted ${id} match, qualifying paired warehouse observation, ranking-proven current evidence snapshot, or verified PBP index match was available for diagnostic sampling.`;
...
export async function runEvidenceCoverageRuntimeDiagnostic(){const metrics=await activeMetrics();if(metrics.length!==81)throw new Error(`Expected 81 active metrics, found ${metrics.length}`);const sample=await representativeMatches();if(!sample.matches.length)throw new Error("No real persisted matches, paired warehouse observations, ranking-proven persisted evidence pairs, or verified PBP index matches were available for evidence coverage sampling");const matches:any[]=[];
  ...
  return {schema_version:10,generated_at:new Date().toISOString(),metrics:metrics.length,sampling:{source:"REAL_PERSISTED_MATCHES_WITH_WAREHOUSE_PERSISTED_PAIR_VERIFIED_INDEX_CERTIFIED_LOCAL_AND_COMPLETION_HISTORICAL_FALLBACK",requested_classes:["ATP_MAIN","WTA_MAIN","ATP_CHALLENGER"],sampled_classes:matches.map(m=>m.id),missing_classes:sample.missing_classes,missing_class_reasons:sample.missing_class_reasons,class_proof:sample.class_proof},matches};
}