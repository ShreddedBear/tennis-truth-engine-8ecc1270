...
function persistedSurface(rows:PersistedPairCandidate[]){for(const row of rows){if(codeOf(row.metric_code)!=="021")continue;const m=String(row.value_text??"").match(/match_surface_(hard|clay|grass|carpet)=1(?:\.0+)?/i);if(m)return m[1].toLowerCase();}return null;}
function persistedPairRepresentative(id:RepresentativeId,rows:PersistedPairCandidate[],index:number):RepresentativeMatch {
  ...
  return {id,match_id:`metric-evidence:${id}:${index}`,p1:row.player_name!,p2:row.opponent_name!,date,date_source:"persisted_as_of_date",tournament,context:[`Evidence sample: persisted metric pair`,`Persisted metrics: ${metricCount}`,`Level: ${level}`,`Tour: ${level}`,surface?`Surface: ${surface}`:null,`Date: ${date}`].filter(Boolean).join(" | "),event_level:level,surface,sampling_source:"metric_evidence_store"};
}
...
async function representativeMatches():Promise<{matches:RepresentativeMatch[];missing_classes:RepresentativeId[];missing_class_reasons:Partial<Record<RepresentativeId,string>>;class_proof:Partial<Record<RepresentativeId,unknown>>}> {
  ...
  if(missing().some(id=>id==="ATP_MAIN"||id==="WTA_MAIN")){
    ...
    if(!persisted.error){
      ...
      const groups=[...grouped.values()].sort((a,b)=>{const ad=String(a[0]?.as_of_date??""),bd=String(b[0]?.as_of_date??"");if(ad!==bd)return bd.localeCompare(ad);const ac=new Set(a.map(r=>codeOf(r.metric_code))).size,bc=new Set(b.map(r=>codeOf(r.metric_code))).size;return bc-ac;});
      for(let index=0;index<groups.length&&missing().some(id=>id==="ATP_MAIN"||id==="WTA_MAIN");index++){const group=groups[index],row=group[0];const inferred=await classifyPairFromExactRankingEvidence(row.player_name!,row.opponent_name!);if(inferred&&missing().includes(inferred))selected.push(persistedPairRepresentative(inferred,group,index));}
    }
  }
  if(missing().length){
    for(const id of missing()){
      ...
      selected.push({id,match_id:row.match_id,p1:row.p1,p2:row.p2,date:row.date,date_source:"verified_index_date",tournament:row.tournament,context:[`Tournament: ${row.tournament}`,`Level: ${level}`,`Tour: ${level}`,row.surface?`Surface: ${row.surface}`:null,`Date: ${row.date}`].filter(Boolean).join(" | "),event_level:level,surface:row.surface,sampling_source:row.sampling_source});
    }
  }
  const class_proof:Partial<Record<RepresentativeId,unknown>>={};
  for(const id of wanted){const proof=await sampleVerifiedEvidenceIndexMatch(id);if(proof)class_proof[id]={match_id:proof.match_id,pair:`${proof.p1} vs ${proof.p2}`,date:proof.date,tournament:proof.tournament,surface:proof.surface};}
  ...
  return {matches:selected,missing_classes:missingClasses,missing_class_reasons,class_proof};
}

function usableEvidenceSide(treatment:unknown,value:unknown){return USABLE.has(String(treatment??""))&&value!==null&&value!==undefined&&String(value).trim()!=="";}
...
async function activeMetrics():Promise<Metric[]>{const {data:doc,error:docError}=await db.from("rule_documents").select("active_version_id").eq("doc_type","METRICS").maybeSingle();if(docError)throw new Error(`metric document lookup: ${docError.message}`);if(!doc?.active_version_id)throw new Error("No active METRICS rule document version");const {data,error}=await db.from("rules").select("rule_code,rule_name,body").eq("version_id",doc.active_version_id).order("rule_code");if(error)throw new Error(`metric rules lookup: ${error.message}`);
  ...
  // predictions -- there is no player-side value to sample for them -- so they are
  // excluded here rather than silently scored as UNAVAILABLE. PROTECTED_UNAVAILABLE
  // codes (a real player metric with a documented determination that no legitimate
...
export async function runEvidenceCoverageRuntimeDiagnostic(){const metrics=await activeMetrics();const expected=playerEvidenceDenominatorCodes().length;if(metrics.length!==expected)throw new Error(`Expected ${expected} active player-level metrics, found ${metrics.length}`);const sample=await representativeMatches();if(!sample.matches.length)throw new Error("No real persisted matches, paired warehouse observations, ranking-proven persisted evidence pairs, or verified PBP index matches were available for evidence coverage sampling");const matches:any[]=[];
  ...
  const runtime_index_status={
    // If this loaded_generated_at is empty/stale after a deploy that changed
    // data/generated/tennis-runtime-index.json, the runtime's readFileSync of
    // that file is failing in production (see runtime-tennis-index-data.server.ts) --
    // every code that depends on it (historical results recovery, WTA/Challenger
    // lookups) will silently fail closed regardless of what code is deployed.
  ...
  return {schema_version:11,generated_at:new Date().toISOString(),runtime_index_status,metrics:metrics.length,sampling:{source:"REAL_PERSISTED_MATCHES_WITH_WAREHOUSE_PERSISTED_PAIR_VERIFIED_INDEX_CERTIFIED_LOCAL_AND_COMPLETION_HISTORICAL_FALLBACK",requested_classes:["ATP_MAIN","WTA_MAIN","ATP_CHALLENGER","WTA_CHALLENGER"],sampled_classes:matches.map(m=>m.id),missing_classes:sample.missing_classes,missing_class_reasons:sample.missing_class_reasons,class_proof:sample.class_proof},matches};
}