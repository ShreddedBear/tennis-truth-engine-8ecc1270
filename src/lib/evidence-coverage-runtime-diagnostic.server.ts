...
type RepresentativeMatch = { id:RepresentativeId; match_id:string; p1:string; p2:string; date:string; date_source:"scheduled_date"|"parsed_summary"|"created_at"|"warehouse_event_date"; tournament:string; context:string; event_level:string|null; surface:string|null; sampling_source:"matches"|"source_observations" };
...
function toRepresentative(id:RepresentativeId,row:MatchCandidate):RepresentativeMatch {
  ...
  return {id,match_id:row.id,p1:row.player1_name,p2:row.player2_name,date,date_source,tournament,context,event_level:row.event_level,surface,sampling_source:"matches"};
}
...
async function representativeMatches():Promise<{matches:RepresentativeMatch[];missing_classes:RepresentativeId[]}> {
  ...
  if(!primary.error){const candidates=await hydrateParsedHints(((primary.data??[]) as MatchCandidate[]).filter(r=>r.player1_name&&r.player2_name));for(const id of wanted){const row=candidates.find(candidate=>classifyTour(candidate)===id);if(row)selected.push(toRepresentative(id,row));}}
...
function unambiguousStoredSide(rows:any[],code:string,player:string,opponent:string){const matching=rows.filter(r=>codeOf(r.metric_code)===code&&evidencePairMatches(r.player_name,r.opponent_name,player,opponent));if(!matching.length)return null;const signatures=new Set(matching.map(r=>JSON.stringify([r.treatment,r.value_text,r.evidence_family,r.unavailable_reason])));return signatures.size===1?matching[0]:null;}
function laneErrorsForPolicy(packet:Record<string,any>,families:string[]){const failures=Array.isArray(packet._query_errors)?packet._query_errors:[];return failures.filter((failure:any)=>Array.isArray(failure?.families)&&failure.families.some((family:string)=>families.includes(family))).map((failure:any)=>`${failure.lane}: ${failure.message}`);}
...
export async function runEvidenceCoverageRuntimeDiagnostic(){const metrics=await activeMetrics();if(metrics.length!==81)throw new Error(`Expected 81 active metrics, found ${metrics.length}`);const sample=await representativeMatches();if(!sample.matches.length)throw new Error("No real persisted matches or paired warehouse observations were available for evidence coverage sampling");const matches:any[]=[];
  for(const sampled of sample.matches){
    ...
    const [identityResult,storedResult,packetResult]=await Promise.allSettled([identityPromise,db.from("metric_evidence_store").select("metric_code,player_name,opponent_name,treatment,evidence_family,value_text,unavailable_reason,valid_until").eq("as_of_date",match.date).in("metric_code",metrics.map(m=>codeOf(m.code))).in("player_name",aliases).in("opponent_name",aliases).or(`valid_until.is.null,valid_until.gt.${now}`),buildMetricObservationContext({metrics,p1:match.p1,p2:match.p2,asOfDate:match.date,tournament:match.tournament})]);
