...
function load() {
  ...
  try {
    cache = ["match_scores_1991-2016.csv", "match_scores_2017.csv"]
      .flatMap((f) => parse(readFileSync(join(process.cwd(), "data/public/datahub-atp", f), "utf8")))
      .filter((r) => yearOf(r) >= HISTORICAL_MIN_YEAR);
  } catch {
    cache = [];
  }
  return cache;
...
function cutoffYear(context: string) {
  const m = context.match(/(?:date\s+)?(20\d{2})-\d{2}-\d{2}/i);
  return m ? Number(m[1]) : null;
}

function stat(p: string, k: string, v: number, n: number): SourcedStat {
  return {
    key: k,
    player: p,
    value: v,
...
export function computeHistoricalScoreProfileStatsFromRows(rows: Row[], player: string, context: string): SourcedStat[] {
  ...
  const ms = rows.filter((r) => {
    const y = yearOf(r);
    return y >= HISTORICAL_MIN_YEAR && (!cy || y < cy) && (norm(r.winner_name ?? "") === pn || norm(r.loser_name ?? "") === pn);
  });
  if (!ms.length) return [];

  let s1 = 0, s1w = 0, s2 = 0, s2w = 0;
...
export function getHistoricalScoreProfileStats(player: string, context: string): SourcedStat[] {
  return computeHistoricalScoreProfileStatsFromRows(load(), player, context);
}
