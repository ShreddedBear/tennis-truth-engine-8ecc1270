...
describe("protected composite DIRECT treatment",()=>{
  it("does not let a named source plus one neighboring statistic make the broad metric DIRECT",()=>{
    ...
    expect(finding.missing_inputs).toEqual(expect.arrayContaining(["scoreline vs expected games","scoreline vs break opportunities","scoreline vs dominance ratio","clutch-performance dependency"]));
  });
