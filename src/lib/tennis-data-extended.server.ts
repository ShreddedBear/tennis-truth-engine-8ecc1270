...
function load(){if(cache)return cache;const out:Match[]=[];try{for(const tour of ["atp","wta"]){const dir=join(ROOT,tour);if(!existsSync(dir))continue;for(const f of readdirSync(dir).filter(x=>/^20\d{2}\.csv$/.test(x))){for(const r of parseCsv(readFileSync(join(dir,f),"utf8"))){if(r.date&&r.winner&&r.loser)out.push({date:r.date,surface:r.surface??"",winner:r.winner,loser:r.loser,score:r.score??"",winnerRank:r.winner_rank??"",loserRank:r.loser_rank??""});}}}}catch{return cache=[];}cache=out.sort((a,b)=>a.date.localeCompare(b.date));return cache;}
