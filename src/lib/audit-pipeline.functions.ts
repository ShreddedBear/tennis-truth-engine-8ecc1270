...
const ZERO_EVIDENCE_RECOVERY_VERSION = "2026-08-22-v1";
...
export const runAuditPipeline = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const [{ makeDeps }, pipeline] = await Promise.all([
      import("./audit-repo.server"),
      import("./audit-pipeline"),
    ]);
    const { runPipeline, STAGES } = pipeline;
    ...
    try {
      ...
      if (latest) {
        ...
        if (falselyTerminal && (structurallyEmpty || badDefinitionCompletion)) {
          ...
          // Recovery for the exact failure visible on the Active Slate: a prior run
          // may have completed/blocked with every metric side finalized UNAVAILABLE
          // before the local historical/reconstruction adapters were wired in.
          // Those terminal row states prevent the improved researcher from ever
          // seeing the metric again. Re-open ONLY unsupported sides once per
          // recovery version, preserving any DIRECT/RECONSTRUCTED/PARTIAL evidence.
          const usableTreatment = (t: unknown) => ["DIRECT", "RECONSTRUCTED", "PARTIAL"].includes(String(t ?? ""));
          const usableSides = metrics.reduce((n, m) =>
            n + (usableTreatment(m["p1_treatment"]) ? 1 : 0) + (usableTreatment(m["p2_treatment"]) ? 1 : 0), 0);
          ...
          if (usableSides === 0 && !alreadyRecovered) {
            for (const row of metrics) {
              const patch: Record<string, unknown> = {};
              if (!usableTreatment(row["p1_treatment"])) {
                patch["p1_status"] = "NOT STARTED";
                patch["p1_treatment"] = null;
                patch["p1_unavailable_reason"] = null;
                patch["p1_provider_error"] = null;
              }
              if (!usableTreatment(row["p2_treatment"])) {
                patch["p2_status"] = "NOT STARTED";
                patch["p2_treatment"] = null;
                patch["p2_unavailable_reason"] = null;
                patch["p2_provider_error"] = null;
              }
              patch["status"] = "NOT STARTED";
              patch["reconstruction_attempted"] = false;
              await deps.update("metric_results", String(row["id"]), patch);
            }

            const restartFrom = STAGES.indexOf("P1 METRIC EXECUTION");
            for (const stage of STAGES.slice(restartFrom)) {
              await deps.setStage(latest.id, data.matchId, stage, {
                status: "PENDING",
                done_count: 0,
                total_count: stage === "P1 METRIC EXECUTION" || stage === "P2 METRIC EXECUTION" ? metrics.length : 0,
                error_code: null,
                error_message: null,
                finished_at: null,
              });
            ...
            await deps.updateRun(latest.id, {
              ...
              independent_inputs: {
                ...(runMeta.independent_inputs ?? {}),
                zero_evidence_recovery_version: ZERO_EVIDENCE_RECOVERY_VERSION,
              },
            });
            await deps.log({
              audit_run_id: latest.id,
              match_id: data.matchId,
              stage: "ZERO EVIDENCE RECOVERY",
              status: "RUNNING",
              output: {
                reason: "Reopened prior UNAVAILABLE metric treatments so newly integrated local historical/reconstruction evidence can be consumed.",
                metric_rows: metrics.length,
                prior_usable_sides: usableSides,
                recovery_version: ZERO_EVIDENCE_RECOVERY_VERSION,
              },
              matrix_visible: false,
            });
          }
