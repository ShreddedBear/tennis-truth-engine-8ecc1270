...
// Treatment is PARTIAL, not RECONSTRUCTED: per docs/metric-audit-001-surface-strength.md
// section 5 (owner decision made in the docs/evidence-work-blockers.md item 3 re-audit
// pass), this engine only ever delivers 2-3 of code 001's 8 named submetrics (Surface
// Elo, Elo Win Probability, and a rough 52-week match count standing in for Surface
// Sample Depth) -- it never computes Effective Weighted Sample, Surface Elo
// Trend/Momentum, Peak Elo vs Current Elo, or either Hard-Court Record bullet. Per this
// project's own HOUSE_RULES bar in audit-research.server.ts ("RECONSTRUCTED is allowed
// only when every required component of the exact definition/formula is sourced"), an
// incomplete submetric set does not qualify for RECONSTRUCTED.
export function computeHistoryMetric(args:{code:HistoryMetricCode;p1:string;p2:string;asOfDate:string;family:EvidenceTourFamily;surface?:string|null;lane:HistoryLane}):HistoryMetricResult|null{const replay=replayElo(args.lane,args.asOfDate),p1Rows=playerRows(replay,args.p1),p2Rows=playerRows(replay,args.p2);if(!p1Rows.length||!p2Rows.length)return null;const currentSurface=surfaceKey(args.surface)||null;let p1:string|null=null,p2:string|null=null,differential:string|null=null;const treatment:"RECONSTRUCTED"|"PARTIAL"="PARTIAL",reliability=86,unavailableReason:string|null=null;let window="strict pre-match chronology",calculation="deterministic K=32 Elo replay";
