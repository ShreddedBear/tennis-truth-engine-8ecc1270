...
async function loadCandidateRows(player:string,opponent:string,asOfDate:string){
  ...
  const pbpBase=()=>db.from("source_observations").select(select).eq("event_date",asOfDate).in("observation_type",["POINT_BY_POINT","PBP"]).order("event_date",{ascending:false});

  // Keep player lanes independent: one failed lookup must not erase evidence
  // from the other player. Native match imports may store either requested
  // player in opponent_name, so MATCH_RESULT_OR_SCHEDULE gets reverse lanes too.
  const laneResults=await Promise.all([
    ...
    pbpBase().in("player_name",p1Aliases).limit(1000),
    pbpBase().in("player_name",p2Aliases).limit(1000),
  ...
  // Generic Evidence Coverage never consumes raw PBP from these lanes; approved
  // PBP enters only through the four tour-scoped BSD bridges below. The PBP
  // lanes exist solely for independent failure diagnostics.
...
function mergePacketEntry(base:any,pbp:any){if(!base)return pbp;if(!pbp)return base;const observations=[...(base.observations??[]),...(pbp.observations??[])];const seen=new Set<string>();const deduped=observations.filter((row:any)=>{const value=row?.value??{};const matchId=value?.match_id??String(row?.url??"").match(/\/matches\/(\d+)\//)?.[1]??"";const key=row?.family==="POINT_BY_POINT"&&matchId?[row?.family,row?.source,matchId].join("|"):[row?.family,row?.source,row?.player,row?.opponent,row?.player1,row?.player2,row?.event_date,row?.key].join("|");if(seen.has(key))return false;seen.add(key);return true;});return{...base,observed_families:unique([...(base.observed_families??[]),...(pbp.observed_families??[])]),direct_satisfaction_allowed:Boolean(base.direct_satisfaction_allowed||pbp.direct_satisfaction_allowed),observations:deduped.slice(0,80),pbp_tour_guard:pbp.tour_guard??null};}

export async function buildMetricObservationContext(args:{metrics:MetricLike[];p1:string;p2:string;asOfDate:string;context?:string|null;}){const loaded=await loadCandidateRows(args.p1,args.p2,args.asOfDate);const rows=loaded.rows;const laneFailures=loaded.laneFailures;const pbpPacket=await approvedPbpPacket(args,rows);const packet:Record<string,any>={};for(const metric of args.metrics){const code=codeOf(metric.code);const policy=policyForMetric(code);const allowed=rows.filter(row=>metricAllowsObservation(code,row));if(allowed.length){const families=unique(allowed.map(row=>observationFamily(row)).filter(Boolean));const supportOnly=policy.support_only_families??[];const sufficient=policy.sufficient_families??[];packet[code]={metric_name:metric.name,allowed_families:policy.allowed_families,sufficient_families:sufficient,support_only_families:supportOnly,observed_families:families,direct_satisfaction_allowed:families.some(family=>sufficient.includes(family!)),observations:allowed.slice(0,80).map(compactObservation)};}packet[code]=mergePacketEntry(packet[code],pbpPacket[code]);if(!packet[code])delete packet[code];}if(laneFailures.length)packet._query_errors = laneFailures;return packet;}
