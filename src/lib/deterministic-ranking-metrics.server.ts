...
function payload(row: Row) {
  try { return JSON.parse(row.text_value ?? "{}") as Record<string, unknown>; }
  catch { return {}; }
}
...
function points(row: Row) {
  const p = payload(row);
  const n = Number(p.points);
  return Number.isFinite(n) && n >= 0 ? n : null;
}
function sourceRefs(rows: Row[]): SourceRef[] {
  const out: SourceRef[] = [], seen = new Set<string>();
...
function nearest(rows: Row[], asOf: string, targetDays: number) {
  return rows
    .filter(row => row.event_date && days(row.event_date, asOf) >= targetDays)
    .sort((a, b) => Math.abs(days(a.event_date!, asOf) - targetDays) - Math.abs(days(b.event_date!, asOf) - targetDays))[0] ?? null;
}
function rankingSummary(player: string, opponent: string, rows: Row[], asOf: string) {
  const playerRows = rows
    .filter(row => evidenceNameMatches(row.player_name, player, opponent) && row.event_date && row.event_date <= asOf)
    .sort((a, b) => String(b.event_date).localeCompare(String(a.event_date)));
  const current = playerRows[0] ?? null;
  if (!current) return null;
  const currentRank = rank(current);
  if (currentRank === null) return null;
  const r30 = nearest(playerRows, asOf, 30), r90 = nearest(playerRows, asOf, 90), r365 = nearest(playerRows, asOf, 365);
  const movement = (row: Row | null) => { const value = row ? rank(row) : null; return value === null ? null : value - currentRank; };
  const ranks52 = playerRows.filter(row => row.event_date && days(row.event_date, asOf) >= 0 && days(row.event_date, asOf) <= 365).map(rank).filter((x): x is number => x !== null);
  return {
    rank: currentRank,
    ...
    rank_change_365d: movement(r365),
    best_rank_52w: ranks52.length ? Math.min(...ranks52) : currentRank,
    snapshots_52w: ranks52.length,
  };
}
function rankingValue(summary: ReturnType<typeof rankingSummary>) {
...
function familyFromContext(context: string | null | undefined): EvidenceTourFamily | null {
...
function expectedCircuit(context: string | null | undefined, rows: Row[]): "ATP" | "WTA" | null {
  ...
  if (family === "ATP_MAIN" || family === "ATP_CHALLENGER") return "ATP";
  if (family === "WTA_MAIN" || family === "WTA_CHALLENGER") return "WTA";
  const circuits = new Set(rows.map(rowCircuit).filter((value): value is "ATP" | "WTA" => Boolean(value)));
...
async function directRankingFinding(args: { p1: string; p2: string; asOfDate: string; context?: string | null }): Promise<MetricFinding | null> {
  ...
  // Official circuit firewall: ATP Main/Challenger share ATP rankings; WTA Main/WTA125 share WTA rankings.
  // There is deliberately no WTA-Challenger ranking namespace.
  ...
  if (!p1 || !p2) return null; // one-sided ranking evidence fails closed
  return {
    metric_code: "014",
    p1_value: p1,
    p2_value: p2,
...
function historyFinding(args: { code: HistoryMetricCode; p1: string; p2: string; asOfDate: string; context?: string | null }): MetricFinding | null {
  const family = familyFromContext(args.context);
  if (!family) return null;
  const lane = (runtimeIndex as any)?.matchHistory?.[family] as HistoryLane | undefined;
  if (!lane || typeof lane !== "object") return null;
  const result = computeHistoryMetric({ code: args.code, p1: args.p1, p2: args.p2, asOfDate: args.asOfDate, family, surface: surfaceFromContext(args.context), lane });
  ...
  return {
    ...
    evidence_family: args.code === "007" || args.code === "061" ? "RESULTS_SCHEDULE" : "RANKING_FORM",
    reliability: result.reliability,
    sample: result.sample,
    unavailable_reason: result.unavailable_reason,
    sources: result.source_names.map(source_name => ({ source_name, url: null, retrieved_at: null })),
  };
}
...
export async function deterministicRankingMetric(args: { metricCode: string; p1: string; p2: string; asOfDate: string; context?: string | null }): Promise<MetricFinding | null> {
  ...
  if (!HISTORY_CODES.has(code as HistoryMetricCode)) return null;
  return historyFinding({ ...args, code: code as HistoryMetricCode });
}
