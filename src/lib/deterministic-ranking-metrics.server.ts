...
// Rapid Riser/Faller Status (public/seed/metrics.txt #14's third bullet):
// "whether a player is moving unusually quickly up or down the rankings."
// A transparent, documented threshold on the already-computed 30-day rank
// delta -- 20+ ranking positions gained is RAPID_RISER, 20+ lost is
// RAPID_FALLER, otherwise STABLE. Negative rank_change_30d means the
// player's rank number dropped (improved); positive means it rose (worse).
export const RAPID_RANK_MOVE_THRESHOLD = 20;
export function rapidStatus(rankChange30d: number | null): "RAPID_RISER" | "RAPID_FALLER" | "STABLE" | null {
  if (rankChange30d === null) return null;
  if (rankChange30d <= -RAPID_RANK_MOVE_THRESHOLD) return "RAPID_RISER";
  if (rankChange30d >= RAPID_RANK_MOVE_THRESHOLD) return "RAPID_FALLER";
  return "STABLE";
}
function rankingSummary(player: string, opponent: string, rows: Row[], asOf: string) { const playerRows = rows.filter(row => evidenceNameMatches(row.player_name, player, opponent) && row.event_date && row.event_date <= asOf).sort((a, b) => String(b.event_date).localeCompare(String(a.event_date))); const current = playerRows[0] ?? null; if (!current) return null; const currentRank = rank(current); if (currentRank === null) return null; const r30 = nearest(playerRows, asOf, 30), r90 = nearest(playerRows, asOf, 90); const movement = (row: Row | null) => { const value = row ? rank(row) : null; return value === null ? null : value - currentRank; }; const rankChange30d = movement(r30); const ranks52 = playerRows.filter(row => row.event_date && days(row.event_date, asOf) >= 0 && days(row.event_date, asOf) <= 365).map(rank).filter((x): x is number => x !== null); return { rank: currentRank, points: points(current), observation_date: current.event_date!, rank_change_30d: rankChange30d, rank_change_90d: movement(r90), rapid_status: rapidStatus(rankChange30d), best_rank_52w: ranks52.length ? Math.min(...ranks52) : currentRank, snapshots_52w: ranks52.length }; }
function rankingValue(summary: ReturnType<typeof rankingSummary>) { if (!summary) return null; return `rank=${summary.rank}; points=${summary.points ?? "NA"}; observation_date=${summary.observation_date}; rank_change_30d=${summary.rank_change_30d ?? "NA"}; rank_change_90d=${summary.rank_change_90d ?? "NA"}; rapid_status=${summary.rapid_status ?? "NA"}; best_rank_52w=${summary.best_rank_52w}; snapshots_52w=${summary.snapshots_52w}`; }
...
// Treatment is PARTIAL, not DIRECT: per docs/metric-audit-014-ranking-context.md
// (this project's own house rule in audit-research.server.ts -- "DIRECT" requires
// "the exact metric itself is published at a named source"), only this composite
// metric's own first bullet (Current Ranking) is a raw published value. Ranking
// Trajectory and Rapid Riser/Faller Status are now genuinely computed from that
// same published history (not fabricated), but Ranking–Performance Disconnect
// (the gap between ranking and what underlying performance implies) needs a
// performance baseline -- e.g. derived Elo -- this file does not cross-reference,
// so the full four-bullet definition is not "published at a named source" as such.
async function directRankingFinding(args: { p1: string; p2: string; asOfDate: string; context?: string | null }): Promise<MetricFinding | null> { const start = new Date(`${args.asOfDate}T00:00:00Z`); start.setUTCFullYear(start.getUTCFullYear() - 2); const fetched = await rankingRows(args.p1, args.p2, start.toISOString().slice(0, 10), args.asOfDate); if (!fetched) return null; const circuit = expectedCircuit(args.context, fetched); if (!circuit) return null; const rows = fetched.filter(row => metricAllowsObservation("014", row) && rowCircuit(row) === circuit && row.event_date && row.event_date <= args.asOfDate); if (!rows.length) return null; const p1Summary = rankingSummary(args.p1, args.p2, rows, args.asOfDate), p2Summary = rankingSummary(args.p2, args.p1, rows, args.asOfDate); const p1 = rankingValue(p1Summary), p2 = rankingValue(p2Summary); if (!p1 || !p2) return null; return { metric_code: "014", p1_value: p1, p2_value: p2, p1_treatment: "PARTIAL", p2_treatment: "PARTIAL", differential: p1Summary && p2Summary ? `ranking_gap_p1_minus_p2=${p1Summary.rank - p2Summary.rank}` : null, evidence_family: "RANKING", reliability: 95, sample: `source_observations=official ${circuit} ranking snapshots; date_window=observation_date<=${args.asOfDate}; players=${args.p1} vs ${args.p2}; calculation=latest valid ranking plus historical trend plus rapid-riser/faller threshold; output=pair-complete; metric=014; circuit=${circuit}; match_date=${args.asOfDate}; future_leakage=blocked`, unavailable_reason: "Current Ranking, Ranking Trajectory, and Rapid Riser/Faller Status are covered from official ranking snapshots; Ranking-Performance Disconnect requires a performance baseline (e.g. derived Elo) this path does not cross-reference and is not inferred.", sources: sourceRefs(rows) }; }
