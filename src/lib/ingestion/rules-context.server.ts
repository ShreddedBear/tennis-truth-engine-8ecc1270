import { enabledTargets, markTargetIngested, upsertObservations } from "./warehouse-repo.server";
...
export async function ingestRulesContext(source:RulesSource){
  ...
  return {source,targets:targets.length,pages_read,observations_written};
}
