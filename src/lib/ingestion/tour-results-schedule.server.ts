...
function embeddedJson(html: string): unknown[] {
  ...
  for (const match of scripts) {
    ...
    try { values.push(JSON.parse(body)); } catch { /* non-JSON script */ }
  }
  return values;
}

function sourceMatchesTour(source: TourSource, obj: Record<string, unknown>) {
  if (source !== "atp_challenger") return true;
  const level = first(obj, ["level", "eventLevel", "category", "tournamentLevel", "type"]);
  return !level || /challenger/i.test(level);
}
...
function normalizedFromObject(source: TourSource, url: string, target: Target, obj: Record<string, unknown>): Observation[] {
  ...
  const player1 = first(obj, ["player1", "playerOne", "homePlayer", "competitor1", "winnerName", "playerA", "participant1"]);
  const player2 = first(obj, ["player2", "playerTwo", "awayPlayer", "competitor2", "loserName", "playerB", "participant2"]);
  const tournament = first(obj, ["tournament", "tournamentName", "event", "eventName", "competitionName", "name"]);
  const dateRaw = first(obj, ["date", "matchDate", "startDate", "startTime", "scheduledAt", "eventDate"]);
  const eventDate = isoDate(dateRaw);
  const surface = first(obj, ["surface", "courtSurface"]);
  const round = first(obj, ["round", "roundName", "stage"]);
  const status = first(obj, ["status", "matchStatus", "state"]);
  const score = first(obj, ["score", "scoreText", "result"]);
  const winner = first(obj, ["winner", "winnerName"]);
  ...
  const common = {
    ...
    raw_payload: obj,
    provenance: { target_key: target.target_key, tour: source, extraction: "official_page_structured_json" },
  };
  ...
  if (looksLikeMatch) {
    rows.push({
      ...
      text_value: JSON.stringify({ player1, player2, round, status, score, winner }),
      numeric_value: null,
    });
  }
  if (looksLikeSchedule) {
    rows.push({
      ...
      text_value: JSON.stringify({ tournament, date: eventDate ?? dateRaw, surface, round, status }),
      numeric_value: null,
    });
  }
  return rows;
...
async function fetchStructured(url: string) {
  ...
  return embeddedJson(await res.text());
}
...
export async function ingestTourResultsAndSchedules(source: TourSource) {
  const { data: targets, error } = await db
    ...
    .select("id,source_id,target_key,pullback_start,pullback_end,config")
    .eq("source_id", source)
    .eq("enabled", true);
  if (error) throw error;
  ...
  for (const target of (targets ?? []) as Target[]) {
    ...
    const url = typeof config.url === "string" && config.url ? config.url : DEFAULT_URLS[source];
    const payloads = await fetchStructured(url);
    pagesRead += 1;

    const objects: Record<string, unknown>[] = [];
    for (const payload of payloads) collectObjects(payload, objects);
    structuredObjectsSeen += objects.length;
    ...
    await db.from("ingestion_targets")
