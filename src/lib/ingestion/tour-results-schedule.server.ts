...
function isChallengerLevel(level:string|null) {
...
function normalizedFromObject(source:TourSource,url:string,target:Target,obj:Record<string,unknown>):Observation[] {
  ...
  const common={
    ...
    raw_payload:obj,
    provenance:{target_key:target.target_key,tour:source,level:levelText(obj),extraction:url.includes("protennislive")?"official_atp_feed":"official_page_structured_json"},
  };
...
async function writeRows(rows:Observation[]) {
  ...
  for (let i=0;i<rows.length;i+=500) {
    ...
    const keys=chunk.map((row)=>row.source_record_key);
...
export async function ingestTourResultsAndSchedules(source:TourSource) {
  const {data:targets,error}=await db.from("ingestion_targets").select("id,source_id,target_key,pullback_start,pullback_end,config").eq("source_id",source).eq("enabled",true);
  if (error) throw error;
  ...
  for (const target of (targets??[]) as Target[]) {
    ...
    const fetched=await fetchStructured(source,url);
    pagesRead++;
    const objects:Record<string,unknown>[]=[];
    for (const payload of fetched.payloads) collectObjects(payload,objects);
    structuredObjectsSeen+=objects.length;
    const dedupe=new Map<string,Observation>();
    for (const obj of objects) for (const row of normalizedFromObject(source,fetched.url,target,obj)) dedupe.set(row.source_record_key,row);
