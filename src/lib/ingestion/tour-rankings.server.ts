...
function normalize(source:RankingSource,url:string,target:Target,o:Record<string,unknown>):Row|null{
  ...
  const row:Row={
    ...
    raw_payload:o, provenance:{target_key:target.target_key,tour:source,extraction:url.includes("api.wtatennis.com")?"official_wta_public_api":"official_page"},
  };
  assertObservationFamily(row,"RANKING");
  return row;
}

function parseRankingTable(html:string,source:RankingSource,target:Target,url:string):Row[]{
  ...
  for(const tr of html.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)){
    ...
    let player=(cells[1]??"").replace(/\b[A-Z]{3}\b/g,"").replace(/^[-+\d\s]+/,"").trim();
    if(!/[A-Za-zÀ-ž]/.test(player)) continue;
    let points:number|null=null;
    if(source==="atp_rankings") points=num(cells[2]); else points=num(cells[cells.length-1]);
    const key=`${rank}:${player}`; if(seen.has(key)) continue; seen.add(key);
    const row=normalize(source,url,target,{playerName:player,rank,points,rankingDate}); if(row) rows.push(row);
  }
  return rows;
}
...
async function fetchRows(source:RankingSource,url:string,target:Target){
  ...
  const html=await r.text(); const payloads=embedded(html); const objects:Record<string,unknown>[]=[]; for(const p of payloads)collect(p,objects);
  const rows=new Map<string,Row>(); for(const o of objects){const row=normalize(source,effectiveUrl,target,o);if(row)rows.set(row.source_record_key,row);} for(const row of parseRankingTable(html,source,target,effectiveUrl))rows.set(row.source_record_key,row);
  if(rows.size || source!=="wta_rankings") return {rows:[...rows.values()],objects_seen:objects.length,pages:1};

  const api=await request(WTA_RANKINGS_API); if(!api.ok) throw new Error(`${WTA_RANKINGS_API} returned ${api.status}`); const payload=await api.json() as unknown; const apiObjects:Record<string,unknown>[]=[]; collect(payload,apiObjects);
  for(const o of apiObjects){const row=normalize(source,WTA_RANKINGS_API,target,o);if(row)rows.set(row.source_record_key,row);} return {rows:[...rows.values()],objects_seen:objects.length+apiObjects.length,pages:2};
}
...
export async function ingestTourRankings(source:RankingSource){
  ...
  for(const target of (targets??[]) as Target[]){
    const cfg=target.config??{}; const url=typeof cfg.url==="string"&&cfg.url?cfg.url:DEFAULT_URLS[source]; const fetched=await fetchRows(source,url,target); pages_read+=fetched.pages; objects_seen+=fetched.objects_seen;
