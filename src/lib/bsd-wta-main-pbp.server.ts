...
async function loadIndex(y:number):Promise<IndexRow[]>{try{const p=JSON.parse(await readFile(join(process.cwd(),"data","audit","bsd-wta-main-pbp-history",String(y),"results.json"),"utf8"));return Array.isArray(p)?p:[];}catch{return[];}}
...
async function fetchPbp(id:string|number):Promise<PbpFetchResult>{
 ...
 try{
  const r=await fetch(`${BASE}/matches/${encodeURIComponent(String(id))}/point-by-point/`,{headers:{Authorization:`Token ${token}`,"User-Agent":"tennis-truth-engine-task18b-wta-main/1.0"},signal:AbortSignal.timeout(12000)});
...
async function computeObservations(args:{p1:string;p2:string;asOfDate:string;context?:string|null}):Promise<{status:ObservationStatus;observations:any[]}>{
 ...
 const promise=(async():Promise<{status:ObservationStatus;observations:any[]}>=>{
  const status:ObservationStatus={eligible:false,reason:"",matches_used:0,rejected_pbp:0,coverage_start:COVERAGE_START,source:"BSD/Bzzoiro WTA Main PBP",fetch_failures:0,fetch_failure_sample:null};
  ...
  status.reason=observations.length?"Approved WTA Main PBP reconstructed through canonical match identity where metric-specific raw fields are satisfied.":"No matching approved WTA Main PBP satisfied Task 18B field requirements.";
