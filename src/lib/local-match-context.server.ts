...
function cleanTournament(v:string|null|undefined){if(!v)return null;const raw=v.replace(/^\$?[\d,]+\s*(?:vol(?:ume)?)?\s*/i,"").trim();const n=norm(raw);if(/cincinn/.test(n))return"Cincinnati Open";if(/montreal|canadian open|rogers cup/.test(n))return"Canadian Open";if(/us open/.test(n))return"US Open";return raw||null;}
function hintTournament(hints:Fields){return cleanTournament(hints.tournament??hints.event??null);}
function hintDate(hints:Fields){const v=hints.scheduled_date??hints.date??null;if(!v)return null;const s=String(v);const iso=s.match(/20\d{2}-\d{2}-\d{2}/)?.[0];if(iso)return iso;const today=new Date().toISOString().slice(0,10);if(/\btoday\b/i.test(s))return today;const tomorrow=new Date(Date.now()+86400000).toISOString().slice(0,10);if(/\btomorrow\b/i.test(s))return tomorrow;return null;}
...
export function resolveLocalMatchContext(p1:string,p2:string,hints:Fields){
  ...
  const surface=(direct?.surface||hints.surface||registry.surface||null)?.trim()||null;
  const round=normalizeRound(direct?.round||hints.round||null);
  const date=direct?.date||hintDate(hints)||null;
  const level=direct?eventLevelFromRow(direct):(hints.event_level??registry.event_level??null);
  const fields:Fields={tournament:registry.tournament??tournament,event_level:level,round,scheduled_date:date,surface,best_of:hints.best_of??registry.best_of??bestOfFromContext(level,tournament,tour??"ATP")};
