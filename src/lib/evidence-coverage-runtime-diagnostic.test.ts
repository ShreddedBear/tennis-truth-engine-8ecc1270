...
describe("runtime evidence coverage diagnostic", () => {
  ...
  it("keeps generic PBP excluded and uses approved tour-scoped bridges", () => {
    expect(diagnostic).not.toContain("deterministicPbpMetric");
    expect(researcher).not.toContain("deterministicPbpMetric({");
    expect(researcher).toContain("buildBsdAtpMainPbpContext");
    expect(researcher).toContain("buildBsdWtaMainPbpContext");
    expect(researcher).toContain("buildBsdAtpChallengerPbpContext");
    expect(researcher).toContain("buildBsdWtaChallengerPbpContext");
  });
  ...
  it("bounds runtime diagnostic database concurrency", () => {
    expect(diagnostic).toContain("DIAGNOSTIC_QUERY_CONCURRENCY");
    expect(diagnostic).toContain("Promise.all(chunk.map");
  });

  it("keeps the temporary endpoint obscure and no-store", () => {
    expect(route).toContain("ECOV-20260825-b6f1");
    expect(route.toLowerCase()).toContain('cache-control');
