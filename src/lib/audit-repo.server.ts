  ...
  return {
    ...
    async getParsedFields(matchId) {
      ...
      const { data } = await db
        ...
        .eq("summary_version_id", sv.id);
      const out: Record<string, string> = {};
      for (const row of data ?? []) {
        const v = row.normalized_value ?? row.raw_value;
        if (v) out[row.field_key] = v;
      }
      return out;
    },
    ...
    async getRules(versionId) {
      const { data } = await db
        .from("rules")
        .select("id, rule_code, rule_name, body, severity, blocking")
    ...
    async getLatestRun(matchId) {
      const { data } = await db
        ...
        .limit(1);
      return ((data?.[0] as never) ?? null) as RunRow | null;
    },
    async createRun(row) {
      const { data, error } = await db
        ...
        .select()
        .single();
      if (error || !data) throw new Error(`Could not create audit run: ${error?.message}`);
      return data as never;
    },
    async updateRun(runId, patch) {
      const { error } = await db.from("audit_runs").update(patch as never).eq("id", runId);
      if (error) throw new Error(`Database update failed (audit_runs): ${error.message}`);
    },
    async list(table: ChildTable, runId) {
      const { data, error } = await db.from(table).select("*").eq("audit_run_id", runId);
      if (error) throw new Error(`Database read failed (${table}): ${error.message}`);
      return (data ?? []) as never;
    },
    async insert(table: ChildTable, rows) {
      for (let i = 0; i < rows.length; i += 200) {
        const { error } = await db.from(table).insert(rows.slice(i, i + 200).map((r) => ({ ...r, user_id })) as never);
        if (error) throw new Error(`Database insert failed (${table}): ${error.message}`);
      }
    },
    async update(table: ChildTable, id, patch) {
      const { error } = await db.from(table).update(patch as never).eq("id", id);
      if (error) throw new Error(`Database update failed (${table}): ${error.message}`);
    },
    async getStages(runId) {
      const { data } = await db
        .from("audit_stage_runs")
        .select("stage, status, attempts, error_message, done_count, total_count")
        .eq("audit_run_id", runId);
      return (data ?? []) as never;
    },
    async setStage(runId, matchId, stage: Stage, patch) {
      const { error } = await db.from("audit_stage_runs").upsert(
        {
          audit_run_id: runId,
          match_id: matchId,
          stage,
        ...
        { onConflict: "audit_run_id,stage" },
      );
      if (error) throw new Error(`Database write failed (audit_stage_runs): ${error.message}`);
    },
    async saveIdentityRecords(matchId, rows) {
      await db.from("match_identity_records").delete().eq("match_id", matchId).in(
        "field",
        rows.map((r) => String(r["field"])),
      );
      await db.from("match_identity_records").insert(rows.map((r) => ({ ...r, match_id: matchId, user_id })) as never);
    },
    async saveSnapshots(runId, rows) {
      if (!rows.length) return;
      await db.from("source_snapshots").insert(rows.map((r) => ({ ...r, audit_run_id: runId, user_id })) as never);
    },
    async saveConflicts(runId, rows) {
      if (!rows.length) return;
      await db.from("source_conflicts").insert(rows.map((r) => ({ ...r, audit_run_id: runId, user_id })) as never);
    },
    async getCalibration() {
      ...
      const { data: buckets } = await db
        ...
        .select("bucket_code, wp_min, wp_max, wins, graded")
        .eq("calibration_version_id", version.id)
        .order("wp_min");
      return { version, buckets: (buckets ?? []) as never };
    },
    async getDecisionId(runId) {
      const { data } = await db.from("final_decisions").select("id").eq("audit_run_id", runId).maybeSingle();
      return data?.id ?? null;
    },
    async saveDecision(runId, existingId, payload) {
      if (existingId) {
        const { error } = await db.from("final_decisions").update(payload as never).eq("id", existingId);
        if (error) throw new Error(`Database update failed (final_decisions): ${error.message}`);
      } else {
        const { error } = await db.from("final_decisions").insert({ ...payload, user_id } as never);
        if (error) throw new Error(`Database insert failed (final_decisions): ${error.message}`);
      }
    },
    async getConflicts(runId) {
      const { data } = await db.from("source_conflicts").select("critical, resolution_status").eq("audit_run_id", runId);
      return (data ?? []) as never;
    },
