...
export function reconstructPbpScoreState(payload:any):PbpRecovery{
 ...
 for(const s of SIDES){const t=totals[s],add=(code:string,treatment:"RECONSTRUCTED"|"PARTIAL",value:RecoveredMetric["value"],raw:string[],transform:string)=>{derived[s][code]={treatment,value,raw_fields:raw,transformation:transform}};
  ...
  // Treatment is PARTIAL, not RECONSTRUCTED: real code 018 has 3 named bullets
  // (Performance Following Momentum Events -- explicitly "after breaking, being
  // broken, OR winning/losing a tiebreak"; Closing Ability; Opponent Comeback
  // Susceptibility). This replay only covers the "being broken" half of the first
  // bullet (not the tiebreak-momentum half) and Closing Ability fully; Opponent
  // Comeback Susceptibility (the *opponent's* tendency to allow comebacks) isn't
  // computed at all. Per this project's own house rule (RECONSTRUCTED requires
  // every named component), 2 of 3 bullets -- one only partially -- doesn't
  // qualify. See docs/metric-audit-batch-005-016-018-020-032-068.md.
  if(hasSetBoundaries){add("018","PARTIAL",{breakback_opportunities:t.breakbackOpportunities,breakbacks:t.breakbacks,breakback_rate_pct:pct(t.breakbacks,t.breakbackOpportunities),closeout_opportunities:t.closeoutOpportunities,closeouts:t.closeouts,closeout_rate_pct:pct(t.closeouts,t.closeoutOpportunities)},["server","game winner","set boundary","chronological game order"],"Grade the immediate return game after being broken (breakback) and serving-for-set closeouts within the same set. Retargeted from the mismatched codes 070/071 to real code 018 (\"Momentum & Closing Metrics\"), whose own bullets name \"Performance Following Momentum Events\" (breaking/being broken/tiebreak), \"Closing Ability\" (serving-for-match/set position), and \"Opponent Comeback Susceptibility\". This replay covers breakback (the being-broken half of the first bullet, not the tiebreak-momentum half) and closeout rate; the tiebreak-momentum component and Opponent Comeback Susceptibility are not computed, so treatment is PARTIAL rather than the composite's full RECONSTRUCTED bar.")}
