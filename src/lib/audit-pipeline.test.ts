...
describe("Run Audit pipeline", () => {
  ...
  it("instantiates every META_OR_NON_PLAYER code as EXCLUDED and never asks research for it", async () => {
    ...
    // player metric, sent to research as before -- except the 15 codes genuinely
    // classified PROTECTED_UNAVAILABLE in the real canonical registry (017, 022, 054,
    // 063, 065, 066, 067, 069, 072, 073, 074, 076, 078, 079, 081 -- "022" added
    // 2026-08-29, see docs/metric-audit-021-022-surface-environment-and-shot-level.md),
    // which this pipeline correctly settles as NO_SOURCE before ever reaching research,
    // same as META_OR_NON_PLAYER. "M70" is excluded here too, on top of those 15: this
    // test file's module-level mock (see the top of this file) additionally treats real
    // code 070 as PROTECTED_UNAVAILABLE for the dedicated test below, and that mock is
    // file-scoped, so it also applies here.
    const realProtectedUnavailableCodes = ["017", "022", "054", "063", "065", "066", "067", "069", "072", "073", "074", "076", "078", "079", "081"].map((suffix) => `M${suffix.replace(/^0/, "")}`);
