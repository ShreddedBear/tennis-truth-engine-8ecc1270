...
function parsePayload(row:RepositoryResultsObservation){const p=row.raw_payload??{};const d=(p.history_detail&&typeof p.history_detail==="object"?p.history_detail:{}) as Record<string,unknown>;const n=(v:unknown)=>{const x=Number(v);return Number.isFinite(x)&&v!==null&&v!==""?x:null;};const pairs=Array.isArray(d.set_scores)?d.set_scores.flatMap(v=>Array.isArray(v)&&v.length>=2&&Number.isFinite(Number(v[0]))&&Number.isFinite(Number(v[1]))?[[Number(v[0]),Number(v[1])] as [number,number]]:[]):[];return {winner:String(p.winner??"")||null,detail:{setsFor:n(d.sets_for),setsAgainst:n(d.sets_against),setScores:pairs,bestOf:n(d.best_of),opponentRank:n(d.opponent_rank),opponentElo:n(d.opponent_elo),status:d.status==null?null:String(d.status)}};}
function historicalRows(rows:RepositoryResultsObservation[]):HistoricalResultRow[]{return rows.flatMap(row=>{if(!row.event_date||!row.player_name||!row.opponent_name)return[];const parsed=parsePayload(row);const won=parsed.winner===row.player_name?true:parsed.winner===row.opponent_name?false:null;return[{date:row.event_date,player:row.player_name,opponent:row.opponent_name,won,surface:row.surface,tournament:row.tournament,...parsed.detail}];});}
...
export async function deterministicHistoricalResultsMetric(args:{metricCode:string;p1:string;p2:string;asOfDate:string;tournament?:string|null;surface?:string|null;tourFamily?:EvidenceTourFamily|null;context?:string|null;}):Promise<MetricFinding|null>{
  ...
  const allObs=[...p1Rows,...p2Rows];const rows=historicalRows(allObs);
  const a=deriveHistoricalResultMetric({code,player:args.p1,opponent:args.p2,rows,asOfDate:args.asOfDate,surface:args.surface});const b=deriveHistoricalResultMetric({code,player:args.p2,opponent:args.p1,rows,asOfDate:args.asOfDate,surface:args.surface});if(!a||!b)return null;
