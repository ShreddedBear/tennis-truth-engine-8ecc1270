...
export async function deterministicHistoricalResultsMetric(args:{metricCode:string;p1:string;p2:string;asOfDate:string;tournament?:string|null;surface?:string|null;tourFamily?:EvidenceTourFamily|null;context?:string|null;}):Promise<MetricFinding|null>{
  ...
  const allObs=[...p1Rows,...p2Rows];const rows=historicalRows(allObs);const surface=args.surface??contextSurface(args.context);
  const a=deriveHistoricalResultMetric({code,player:args.p1,opponent:args.p2,rows,asOfDate:args.asOfDate,surface});const b=deriveHistoricalResultMetric({code,player:args.p2,opponent:args.p1,rows,asOfDate:args.asOfDate,surface});if(!a||!b)return null;
