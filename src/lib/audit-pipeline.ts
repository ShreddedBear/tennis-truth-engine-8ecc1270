...
try{findings=await deps.research.metrics({p1:match.player1_name,p2:match.player2_name,context:digestContext,auditDate:match.scheduled_date,dossier,researchSide:side,researchPlayer:side==="p1"?match.player1_name:match.player2_name,researchOpponent:side==="p1"?match.player2_name:match.player1_name,metrics:batch.map(r=>({code:String(r["metric_code"]),name:String(r["metric_name"]),body:bodyByCode.get(String(r["metric_code"]))??null}))});}catch(error){providerError=errorDetail(error);}
console.log(`[research-timing] ${side.toUpperCase()} batch of ${batch.length} ${providerError?"ERROR":"OK"} ${Date.now()-batchStartedAt}ms`);const byCode=new Map(findings.map(f=>[f.metric_code,f]));const retrievedAt=deps.now().toISOString();const updates=batch.map(row=>{const paired=metricPairPatch(byCode.get(String(row["metric_code"])),providerError,retrievedAt);const oriented=preserveSettledOppositeSide(paired,row,side);return deps.update("metric_results",String(row["id"]),preserveUsableCurrentSide(oriented,row,side));});await Promise.all(updates);treatedInPass+=batch.length;await ctx.progress(completedBefore+treatedInPass,rows.length);}
...
async function commitFinalDecision(deps:PipelineDeps,matchId:string,runId:string):Promise<StageOutcome>{
  ...
  const decisionRecord=decisionMatch?buildDecisionRecord({
    ...
    // THE SELECTED PLAYER IS PERSISTED FROM THE DECISION RECORD, not from whichever column
  // happens to hold something. gate_report.deterministic_decision.selected_player is the one
  // authoritative field; audit_runs.independent_winner is only consulted for runs decided
  // before that record existed. final_decisions.final_selection is NEVER read as a player --
  // it holds the recommended ACTION.
  const persistedSelection=decisionRecord?decisionRecord.selected_player:(run?.independent_winner??null);
  await deps.saveDecision(runId,existing,{gate_report:decisionRecord?{deterministic_decision:decisionRecord}:{},final_audit_color:report.color,final_recommendation:report.action,completion_percent:report.completionPercent,audit_complete:report.auditComplete,independent_winner:persistedSelection,independent_range:run?.independent_low!==null&&run?.independent_high!==null?`${run?.independent_low}-${run?.independent_high}`:null,calibrated_range:snapshot.calibratedLow!==null&&snapshot.calibratedHigh!==null?`${snapshot.calibratedLow}-${snapshot.calibratedHigh}`:null,calibration_version_id:snapshot.calibrationVersionId,calibration_bucket:snapshot.bucketCode,verified_win_rate:snapshot.verifiedWinRate,calibration_wins:snapshot.bucketWins,calibration_graded:snapshot.bucketGraded,green_locked:report.greenLocked,green_lock_reasons:report.greenLockReasons,matrix_firewall_valid:report.matrixFirewallValid});
