...
export function metricRowsForSideExecution(rows:Array<Record<string,unknown>>,side:"p1"|"p2",priorDone=0){
  ...
  if(side==="p1"){
    const pending=stableRows.filter(row=>!["COMPLETE","UNAVAILABLE","EXCLUDED","NO_SOURCE"].includes(String(row["p1_status"])));
    return{pending,completedBefore:stableRows.length-pending.length};
  }
  const eligible=stableRows.filter(row=>!["EXCLUDED","NO_SOURCE"].includes(String(row["p2_status"])));
