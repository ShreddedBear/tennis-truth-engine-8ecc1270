...
import { assertSelectedPlayerIdBelongsToMatch, resolveSelectedPlayer } from "./selected-player-identity";
...
async function commitConclusion(deps:PipelineDeps,matchId:string,runId:string):Promise<StageOutcome>{const run=await deps.getLatestRun(matchId);if(run?.independent_decision_committed_at)return{status:"COMPLETE",done:1,total:1};const match=await deps.getMatch(matchId);if(!match)throw new Error("match disappeared");const metrics=await deps.list("metric_results",runId),evidence=digestFrom(match,metrics);
  ...
  // THE SIDE IS COMMITTED WITH THE NAME. deterministic.audit.audit_winner_side is the P1/P2
  // the decision core actually produced; writing only the name here is what forced every
  // downstream consumer to guess the side back from a display string, and is where the
  // canonical identity was first lost. Nothing about WHO is selected changes -- this records
  // one more field of the answer the same conclusion already gave.
  await deps.updateRun(runId,{independent_winner:conclusion.winner,independent_winner_side:deterministic.audit.audit_winner_side,independent_low:conclusion.low,independent_high:conclusion.high,independent_decision_committed_at:deps.now().toISOString(),effective_evidence_count:families.size,raw_signal_count:metrics.filter(m=>m["status"]==="COMPLETE").length});return{status:"COMPLETE",done:1,total:1,detail:{winner:conclusion.winner,families:families.size,rationale:conclusion.rationale?.slice(0,500)??null}};}
...
async function commitFinalDecision(deps:PipelineDeps,matchId:string,runId:string):Promise<StageOutcome>{
  ...
  const decisionRecord=decisionMatch?buildDecisionRecord({
    ...
    // THE CANONICAL SELECTED PLAYER, resolved from the COMMITTED conclusion -- the same
  // decision that set independent_winner -- and never from the colour, from final_selection,
  // from array position, or from a recomputation that might disagree with what was committed.
  // side -> matches.player{1,2}_id and matches.player{1,2}_name, so a P2 winner can never be
  // rendered as P1 by ordering, and an INSUFFICIENT_EVIDENCE run stays empty.
  const selectedPlayer=decisionMatch?resolveSelectedPlayer(decisionMatch as never,{side:run?.independent_winner_side,name:run?.independent_winner}):null;
  // A selected_player_id must belong to one of THIS match's two players. A foreign id is
  // worse than none -- it names a real player who was not selected -- so it is refused.
  if(selectedPlayer&&decisionMatch)assertSelectedPlayerIdBelongsToMatch(selectedPlayer.player_id,decisionMatch as never);
  await deps.saveDecision(runId,existing,{selected_player_id:selectedPlayer?.player_id??null,gate_report:{...(decisionRecord?{deterministic_decision:decisionRecord}:{}),selectedPlayer},final_audit_color:report.color,final_recommendation:report.action,completion_percent:report.completionPercent,audit_complete:report.auditComplete,independent_winner:run?.independent_winner??null,independent_range:run?.independent_low!==null&&run?.independent_high!==null?`${run?.independent_low}-${run?.independent_high}`:null,calibrated_range:snapshot.calibratedLow!==null&&snapshot.calibratedHigh!==null?`${snapshot.calibratedLow}-${snapshot.calibratedHigh}`:null,calibration_version_id:snapshot.calibrationVersionId,calibration_bucket:snapshot.bucketCode,verified_win_rate:snapshot.verifiedWinRate,calibration_wins:snapshot.bucketWins,calibration_graded:snapshot.bucketGraded,green_locked:report.greenLocked,green_lock_reasons:report.greenLockReasons,matrix_firewall_valid:report.matrixFirewallValid});
