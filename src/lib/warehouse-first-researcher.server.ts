...
async function lookup(metricCodes:string[],player:string,opponent:string,date:string){if(!metricCodes.length)return new Map<string,StoredEvidence>();const {data,error}=await db.from("metric_evidence_store").select("metric_code,player_name,opponent_name,treatment,value_text,reliability,sample_label,evidence_family,sources,unavailable_reason,valid_until").in("metric_code",metricCodes).eq("player_name",player).eq("opponent_name",opponent).eq("as_of_date",date).or(`valid_until.is.null,valid_until.gt.${new Date().toISOString()}`);if(error)return new Map<string,StoredEvidence>();return new Map<string,StoredEvidence>((data??[]).map((row:StoredEvidence)=>[codeOf(row.metric_code),row] as [string,StoredEvidence]));}
