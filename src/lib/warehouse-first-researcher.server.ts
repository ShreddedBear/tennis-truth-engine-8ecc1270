...
type StoredEvidence = { metric_code:string; player_name:string; opponent_name:string|null; treatment:MetricFinding["p1_treatment"]; value_text:string|null; reliability:number|null; sample_label:string|null; evidence_family:string|null; sources:MetricFinding["sources"]|null; unavailable_reason:string|null; valid_until:string|null; };
...
async function lookup(metricCodes:string[],player:string,opponent:string,date:string):Promise<Map<string,StoredEvidence>>{if(!metricCodes.length)return new Map<string,StoredEvidence>();const {data,error}=await db.from("metric_evidence_store").select("metric_code,player_name,opponent_name,treatment,value_text,reliability,sample_label,evidence_family,sources,unavailable_reason,valid_until").in("metric_code",metricCodes).eq("player_name",player).eq("opponent_name",opponent).eq("as_of_date",date).or(`valid_until.is.null,valid_until.gt.${new Date().toISOString()}`);if(error)return new Map<string,StoredEvidence>();return new Map<string,StoredEvidence>((data??[]).map((row:StoredEvidence)=>[codeOf(row.metric_code),row] as [string,StoredEvidence]));}
...
export const warehouseFirstResearcher: Researcher = {
  ...
  async metrics(input){
    const {p1,p2,metrics}=input;const date=asOfDate(input.context);const tournament=tournamentFromContext(input.context);const codes=metrics.map(metric=>codeOf(metric.code));
    const [p1Stored,p2Stored]=await Promise.all([lookup(codes,p1,p2,date),lookup(codes,p2,p1,date)]);
    const missing=metrics.filter(metric=>{const code=codeOf(metric.code);const a=p1Stored.get(code);const b=p2Stored.get(code);return !a||!b||!USABLE.has(a.treatment)||!USABLE.has(b.treatment)||!a.value_text||!b.value_text;});
    ...
    for(const metric of metrics){const code=codeOf(metric.code);const a=p1Stored.get(code);const b=p2Stored.get(code);const live=liveByCode.get(code);const deterministic=deterministicByCode.get(code);
      if(a&&b&&USABLE.has(a.treatment)&&USABLE.has(b.treatment)&&a.value_text&&b.value_text){const mergedSources=[...sourcesOf(a),...sourcesOf(b)].filter((source,index,rows)=>rows.findIndex(other=>other.source_name===source.source_name&&other.url===source.url)===index);output.push({metric_code:code,p1_value:a.value_text,p2_value:b.value_text,p1_treatment:a.treatment,p2_treatment:b.treatment,differential:null,evidence_family:a.evidence_family??b.evidence_family,reliability:Math.min(a.reliability??100,b.reliability??100),sample:[a.sample_label,b.sample_label].filter(Boolean).join(" | ")||null,unavailable_reason:null,sources:mergedSources});continue;}
      const chosen=live&&(USABLE.has(live.p1_treatment)||USABLE.has(live.p2_treatment))?live:deterministic??live;if(!chosen)continue;output.push(chosen);
      await Promise.all([saveSide({code,name:metric.name,player:p1,opponent:p2,date,treatment:chosen.p1_treatment,value:chosen.p1_value,reliability:chosen.reliability,sample:chosen.sample,family:chosen.evidence_family,sources:chosen.sources??[],unavailableReason:chosen.unavailable_reason}),saveSide({code,name:metric.name,player:p2,opponent:p1,date,treatment:chosen.p2_treatment,value:chosen.p2_value,reliability:chosen.reliability,sample:chosen.sample,family:chosen.evidence_family,sources:chosen.sources??[],unavailableReason:chosen.unavailable_reason})]);
    }
    return output;
  },
};