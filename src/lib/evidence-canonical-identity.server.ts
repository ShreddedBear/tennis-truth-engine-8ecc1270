...
async function pagedLane(
  ...
  for (let page = 0; page < MAX_PAGES_PER_LANE; page++) {
    ...
    // Two distinct exact surname matches already prove ambiguity; no later row
    // can make the resolution unique, so stopping here is fail-closed and cheap.
...
async function queryCandidates(input: string) {
  ...
  // Canonical proof comes only from warehouse evidence. The application
  // `matches` table is deliberately excluded: an uploaded/app row must never
  // manufacture a canonical identity that the warehouse itself cannot prove.
...
export async function resolveCanonicalEvidenceIdentity(input: string): Promise<CanonicalEvidenceIdentity> {
  ...
  // Any incomplete/erroring warehouse lane means uniqueness was not proven.
