...
function explicitAtpChallengerContext(context: string | null | undefined) {
  const s = norm(context);
  if (!s) return false;
  if (/(^| )wta( |$)/.test(s) || s.includes("wta 125") || s.includes("wta125")) return false;
  if (!/(^| )atp( |$)/.test(s)) return false;
  if (!s.includes("challenger")) return false;
  if (s.includes("atp main") || s.includes("masters 1000") || s.includes("atp 250") || s.includes("atp 500")) return false;
  return true;
}

function strictIndexedAtpChallenger(row: IndexRow) {
  const circuit = String(row.circuit ?? "").trim().toUpperCase();
  const blob = norm(`${row.category ?? ""} ${row.tournament ?? ""}`);
  if (circuit !== "ATP") return false;
  if (!blob.includes("challenger")) return false;
  if (["wta", "itf", "futures", "utr", "satellite", "exhibition"].some(x => blob.includes(x))) return false;
  return row.structurally_present === true;
}

async function loadIndex(year: number): Promise<IndexRow[]> {
  try {
    const path = join(process.cwd(), "data", "audit", "bsd-atp-challenger-pbp-history", String(year), "results.json");
...
async function fetchPbp(matchId: string | number) {
  ...
  try {
    const r = await fetch(`${BASE}/matches/${encodeURIComponent(String(matchId))}/point-by-point/`, {
      headers: { Authorization: `Token ${token}`, "User-Agent": "tennis-truth-engine-bsd-atp-challenger-metrics/1.0" },
...
export async function buildBsdAtpChallengerPbpContext(args: {
  metrics: MetricLike[];
  p1: string;
  p2: string;
  asOfDate: string;
  ...
  const status = { eligible: false, reason: "", matches_used: 0, coverage_start: COVERAGE_START, source: "BSD/Bzzoiro ATP Challenger PBP" };
  if (!explicitAtpChallengerContext(args.context)) {
    status.reason = "Fail-closed tour guard: context is not explicitly ATP Challenger or contains a blocked WTA/main-tour marker.";
    return { packet: {} as Record<string, unknown>, status };
  }
  if (args.asOfDate < COVERAGE_START) {
    status.reason = "Outside confirmed BSD ATP Challenger PBP coverage (starts 2025).";
    return { packet: {} as Record<string, unknown>, status };
  }
  status.eligible = true;
  const endYear = Math.min(2026, Number(args.asOfDate.slice(0, 4)) || 2026);
  const indexes = (await Promise.all(Array.from({ length: endYear - 2025 + 1 }, (_, i) => loadIndex(2025 + i)))).flat();
  const p1n = norm(args.p1), p2n = norm(args.p2);
  const candidates = indexes.filter(row => {
    if (!strictIndexedAtpChallenger(row) || !row.date || row.date.slice(0, 10) > args.asOfDate) return false;
  ...
  for (const row of candidates) {
    ...
    for (const target of [args.p1, args.p2]) {
      ...
      observations.push({
        ...
        source: "BSD/Bzzoiro ATP Challenger PBP",
        ...
        key: "bsd_atp_challenger_pbp_summary",
        value: summary,
        sample: `${summary.totalPoints} point rows; ${summary.gamesObserved} games`,
        provenance: { circuit: "ATP", level: "CHALLENGER", coverage_floor: COVERAGE_START, strict_index_classifier: true },
      });
  ...
  for (const metric of args.metrics) {
    ...
    packet[code] = {
      ...
      tour_guard: "STRICT_ATP_CHALLENGER_ONLY",
    };
  }
  status.reason = observations.length ? "BSD ATP Challenger PBP attached to eligible metric codes." : "No matching usable BSD ATP Challenger PBP observations found for these players.";
